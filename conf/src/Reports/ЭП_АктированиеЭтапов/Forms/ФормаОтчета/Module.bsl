...
Процедура ОтчетТабличныйДокументОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	РезультатОбработкиРасшифровки = ОбработкаРасшифровкиНаСервере(Расшифровка);  
	
	Если РезультатОбработкиРасшифровки <> Неопределено Тогда 
		
		СтандартнаяОбработка = Ложь;
		ФормаРасшифровки = ПолучитьФорму("Отчет.ЭП_АктированиеЭтапов.Форма.ФормаРасшифровки");
		ФормаРасшифровки.ТабличныйДокумент = РезультатОбработкиРасшифровки.ТабличныйДокумент;
		ФормаРасшифровки.ДанныеРасшифровки = РезультатОбработкиРасшифровки.ДанныеРасшифровки;
...
&НаСервере
Функция ОбработкаРасшифровкиНаСервере(Расшифровка)
	
	ТекущийОбъект = РеквизитФормыВЗначение("Отчет");
	
	СтруктураОтбора = ПолучитьРекурсивноСтруктуруОтбора(Расшифровка, ОтчетДанныеРасшифровки);
	
	Если СтруктураОтбора.Свойство("ВыручкаФакт") ИЛИ СтруктураОтбора.Свойство("СубподрядФакт") 
		ИЛИ СтруктураОтбора.Свойство("ФактТрудозатраты") Тогда 
		
		СхемаКомпановкиДанныхРасшифровки = Отчеты.ЭП_АктированиеЭтапов.ПолучитьМакет("МакетРасшифровкиВыручкаФакт");
		
		Если СтруктураОтбора.Свойство("ВыручкаФакт") Тогда
			СхемаКомпановкиДанныхРасшифровки.НаборыДанных[0].Запрос = ЗапросРасшифровкиВыручки(); 
		ИначеЕсли СтруктураОтбора.Свойство("СубподрядФакт") Тогда
			СхемаКомпановкиДанныхРасшифровки.НаборыДанных[0].Запрос = ЗапросРасшифровкиСубподряда();
		Иначе
			СхемаКомпановкиДанныхРасшифровки.НаборыДанных[0].Запрос = ЗапросРасшифровкиТрудозатрат();
		КонецЕсли;
		
		Настройки = СхемаКомпановкиДанныхРасшифровки.НастройкиПоУмолчанию;
		
		Если НЕ СтруктураОтбора.Свойство("Этап") Или ТипЗнч(СтруктураОтбора.Этап) = Тип("Строка") Тогда 
			СхемаКомпановкиДанныхРасшифровки.Параметры.Этап.Значение = Null;
		Иначе
			СхемаКомпановкиДанныхРасшифровки.Параметры.Этап.Значение = СтруктураОтбора.Этап;
		КонецЕсли;
		
		Если НЕ СтруктураОтбора.Свойство("Проект") Или НЕ ЗначениеЗаполнено(СтруктураОтбора.Проект) Тогда 
			СхемаКомпановкиДанныхРасшифровки.Параметры.Проект.Значение = Null;
		Иначе 
			СхемаКомпановкиДанныхРасшифровки.Параметры.Проект.Значение = СтруктураОтбора.Проект;
		КонецЕсли;
		
		Если НЕ СтруктураОтбора.Свойство("ПроектСопровождение") Тогда
			СхемаКомпановкиДанныхРасшифровки.Параметры.Итог.Значение = Истина;
		Иначе
			СхемаКомпановкиДанныхРасшифровки.Параметры.ПроектСопровождение.Значение = СтруктураОтбора.ПроектСопровождение;
		КонецЕсли;
		
		ПериодОтчета = ТекущийОбъект.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы[0].Значение;
		СхемаКомпановкиДанныхРасшифровки.Параметры.НачалоПериода.Значение = ПериодОтчета.ДатаНачала;
		СхемаКомпановкиДанныхРасшифровки.Параметры.КонецПериода.Значение = ПериодОтчета.ДатаОкончания;
		
		ДанныеРасшифровкиКомпоновки = Новый ДанныеРасшифровкиКомпоновкиДанных;
	   
		КомпоновщикМакета = Новый КомпоновщикМакетаКомпоновкиДанных;
		МакетКомпоновки = КомпоновщикМакета.Выполнить(СхемаКомпановкиДанныхРасшифровки, Настройки, ДанныеРасшифровкиКомпоновки);
		
		ПроцессорКомпановкиДанных = Новый ПроцессорКомпоновкиДанных;
		ПроцессорКомпановкиДанных.Инициализировать(МакетКомпоновки,, ДанныеРасшифровкиКомпоновки);
		...
		АдресДанныхДляРасшифровки = ПоместитьВоВременноеХранилище(ДанныеРасшифровкиКомпоновки, УникальныйИдентификатор);
		
		Возврат Новый Структура("ТабличныйДокумент, ДанныеРасшифровки", ТабличныйДокумент, АдресДанныхДляРасшифровки);
...
Функция ЗапросРасшифровкиВыручки()
	Возврат "ВЫБРАТЬ
	        |	Проекты.Ссылка КАК Ссылка
	        |ПОМЕСТИТЬ ВтПроектыСопровождения
	        |ИЗ
	        |	Справочник.Проекты КАК Проекты
	        |ГДЕ
	        |	Проекты.ЭП_ПроектСопровождение
	        ...
	        |	РазделыПроекта.Ссылка КАК Этап,
	        |	РазделыПроекта.ЭП_ДатаПомещенияВАрхив КАК ДатаПомещенияВАрхив,
	        |	ЭП_РеализацияУслуг.Ссылка КАК Ссылка,
	        |	ЭП_РеализацияУслуг.СуммаАкта КАК СуммаАкта
	        |ПОМЕСТИТЬ ВтЭтапыНеСопровождение
	        |ИЗ
	        |	Справочник.РазделыПроекта КАК РазделыПроекта
	        |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Проекты КАК Проекты
	        |		ПО РазделыПроекта.Владелец = Проекты.Ссылка
	        |		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЭП_РеализацияУслуг КАК ЭП_РеализацияУслуг
	        |		ПО (ЭП_РеализацияУслуг.Этап = РазделыПроекта.Ссылка)
	        |ГДЕ
	        |	НЕ Проекты.ЭП_ПроектСопровождение
	        |	И НЕ РазделыПроекта.ПометкаУдаления
	        |	И НЕ Проекты.ПометкаУдаления
	        |	И ЭП_РеализацияУслуг.Проведен
	        |	И (РазделыПроекта.ДатаОкончания МЕЖДУ &НачалоПериода И &КонецПериода
	        |			ИЛИ РазделыПроекта.ЭП_Архивный
	        |				И (РазделыПроекта.ЭП_ДатаПомещенияВАрхив МЕЖДУ &НачалоПериода И &КонецПериода)
	        |			ИЛИ НЕ РазделыПроекта.ЭП_Архивный
	        |				И РазделыПроекта.ДатаОкончания < &КонецПериода)
	        |
	        |ИНДЕКСИРОВАТЬ ПО
	        |	Этап
	        |;
	        ...
	        |////////////////////////////////////////////////////////////////////////////////
	        |ВЫБРАТЬ
	        |	ВтПроектыСопровождения.Ссылка КАК Проект,
	        |	РазделыПроекта.Ссылка КАК Этап,
	        |	РазделыПроекта.ДатаНачала КАК ДатаНачала,
	        |	РазделыПроекта.ДатаОкончания КАК ДатаОкончания,
	        |	ЭП_РеализацияУслуг.Ссылка КАК Ссылка,
	        |	ЭП_РеализацияУслуг.СуммаАкта КАК СуммаАкта
	        |ПОМЕСТИТЬ ВтЭтапыСопровождение
	        |ИЗ
	        |	ВтПроектыСопровождения КАК ВтПроектыСопровождения
	        |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.РазделыПроекта КАК РазделыПроекта
	        |			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЭП_РеализацияУслуг КАК ЭП_РеализацияУслуг
	        |			ПО (ЭП_РеализацияУслуг.Этап = РазделыПроекта.Ссылка)
	        |		ПО ВтПроектыСопровождения.Ссылка = РазделыПроекта.Владелец
	        |ГДЕ
	        |	НЕ РазделыПроекта.ПометкаУдаления
	        |	И ЭП_РеализацияУслуг.Проведен
	        |	И (РазделыПроекта.ДатаОкончания МЕЖДУ &НачалоПериода И &КонецПериода
	        |			ИЛИ РазделыПроекта.ЭП_Архивный
	        |				И (РазделыПроекта.ЭП_ДатаПомещенияВАрхив МЕЖДУ &НачалоПериода И &КонецПериода)
	        ...
	        |ВЫБРАТЬ
	        |	ВтПроектыСопровождения.Ссылка КАК Проект,
	        |	ТехПроекты.Ссылка КАК ТехПроект,
	        |	Этапы.Ссылка КАК Этап,
	        |	Этапы.ЭП_ДатаПомещенияВАрхив КАК ДатаПомещенияВАрхив,
	        |	РеализацияУслуг.Ссылка КАК Ссылка,
	        |	РеализацияУслуг.СуммаАкта КАК СуммаАкта
	        |ПОМЕСТИТЬ ВтЭтапыСопровождение2
	        |ИЗ
	        |	ВтПроектыСопровождения КАК ВтПроектыСопровождения
	        |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ТехническиеПроекты КАК ТехПроекты
	        |			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЭП_РеализацияУслуг КАК РеализацияУслуг
	        |				ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.РазделыПроекта КАК Этапы
	        |				ПО РеализацияУслуг.Этап = Этапы.Ссылка
	        |			ПО ТехПроекты.ЭП_РеализацияУслуг = РеализацияУслуг.Ссылка
	        |		ПО (ТехПроекты.Владелец = ВтПроектыСопровождения.Ссылка)
	        |ГДЕ
	        |	НЕ ТехПроекты.ПометкаУдаления
	        |	И НЕ Этапы.ПометкаУдаления
	        |	И (Этапы.ДатаОкончания МЕЖДУ &НачалоПериода И &КонецПериода
	        |			ИЛИ Этапы.ЭП_Архивный
	        |				И (Этапы.ЭП_ДатаПомещенияВАрхив МЕЖДУ &НачалоПериода И &КонецПериода)
	        |			ИЛИ НЕ Этапы.ЭП_Архивный
	        |				И Этапы.ДатаОкончания < &КонецПериода)
	        |;
	        |
	        |////////////////////////////////////////////////////////////////////////////////
	        |ВЫБРАТЬ
	        |	ВтЭтапыНеСопровождение.Ссылка КАК Акт,
	        |	ВтЭтапыНеСопровождение.СуммаАкта КАК СуммаАкта
	        |ИЗ
	        |	ВтЭтапыНеСопровождение КАК ВтЭтапыНеСопровождение
	        |ГДЕ
	        |	&Итог
	        |	И (НЕ &Проект ЕСТЬ NULL
	        |				И &Проект = ВтЭтапыНеСопровождение.Проект
	        |			ИЛИ &Проект ЕСТЬ NULL)
	        |
	        |ОБЪЕДИНИТЬ ВСЕ
	        |
	        |ВЫБРАТЬ
	        |	ВтЭтапыНеСопровождение.Ссылка,
	        |	ВтЭтапыНеСопровождение.СуммаАкта
	        |ИЗ
	        |	ВтЭтапыНеСопровождение КАК ВтЭтапыНеСопровождение
	        |ГДЕ
	        |	НЕ &Итог
	        |	И ВтЭтапыНеСопровождение.Этап = &Этап
	        |
	        |ОБЪЕДИНИТЬ ВСЕ
	        |
	        |ВЫБРАТЬ РАЗЛИЧНЫЕ
	        |	ВтЭтапыСопровождение.Ссылка,
	        |	ВтЭтапыСопровождение.СуммаАкта
	        |ИЗ
	        |	ВтЭтапыСопровождение КАК ВтЭтапыСопровождение
	        |ГДЕ
	        |	&Итог
	        |	И (НЕ &Проект ЕСТЬ NULL
	        |				И &Проект = ВтЭтапыСопровождение.Проект
	        |			ИЛИ &Проект ЕСТЬ NULL)
	        |
	        |ОБЪЕДИНИТЬ ВСЕ
	        |
	        ...
	        |ГДЕ
	        |	НЕ &Итог
	        |	И ВтЭтапыСопровождение.Этап = &Этап";
КонецФункции

&НаСервереБезКонтекста
Функция ЗапросРасшифровкиСубподряда()
	Возврат "ВЫБРАТЬ
	        |	Проекты.Ссылка КАК Ссылка
	        |ПОМЕСТИТЬ ВтПроектыСопровождения
	        |ИЗ
	        ...
	        |////////////////////////////////////////////////////////////////////////////////
	        |ВЫБРАТЬ
	        |	ПроектыВсе.Ссылка КАК Проект,
	        |	РазделыПроекта.Ссылка КАК Этап,
	        |	ЭП_ПоступлениеУслуг.Ссылка КАК Акт,
	        |	ЭП_ПоступлениеУслуг.Сумма КАК СуммаАкта
	        |ПОМЕСТИТЬ ВтЭтапыНеСопровождение
	        |ИЗ
	        |	Справочник.РазделыПроекта КАК РазделыПроекта
	        |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЭП_ПоступлениеУслуг.Проекты КАК ЭП_ПоступлениеУслугПроекты
	        |			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЭП_ПоступлениеУслуг КАК ЭП_ПоступлениеУслуг
	        |			ПО ЭП_ПоступлениеУслугПроекты.Ссылка = ЭП_ПоступлениеУслуг.Ссылка
	        |		ПО РазделыПроекта.Ссылка = ЭП_ПоступлениеУслугПроекты.Этап
	        |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Проекты КАК ПроектыВсе
	        |		ПО РазделыПроекта.Владелец = ПроектыВсе.Ссылка
	        |ГДЕ
	        |	НЕ ПроектыВсе.ЭП_ПроектСопровождение
	        |	И НЕ РазделыПроекта.ПометкаУдаления
	        |	И НЕ ПроектыВсе.ПометкаУдаления
	        |	И (РазделыПроекта.ДатаОкончания МЕЖДУ &НачалоПериода И &КонецПериода
	        |			ИЛИ РазделыПроекта.ЭП_Архивный
	        |				И (РазделыПроекта.ЭП_ДатаПомещенияВАрхив МЕЖДУ &НачалоПериода И &КонецПериода)
	        |			ИЛИ НЕ РазделыПроекта.ЭП_Архивный
	        |				И РазделыПроекта.ДатаОкончания < &КонецПериода)
	        |	И ЭП_ПоступлениеУслуг.Проведен
	        |
	        |ИНДЕКСИРОВАТЬ ПО
	        |	Этап
	        |;
	        ...
	        |	ВтПроектыСопровождения.Ссылка КАК Проект,
	        |	ТехПроекты.Ссылка КАК ТехПроект,
	        |	Этапы.ДатаНачала КАК ДатаНачала,
	        |	Этапы.ДатаОкончания КАК ДатаОкончания,
	        |	Этапы.Ссылка КАК Этап,
	        |	Этапы.ЭП_ДатаПомещенияВАрхив КАК ДатаПомещенияВАрхив
	        |ПОМЕСТИТЬ ВтЭтапыСопровождениеБезПост
	        |ИЗ
	        |	ВтПроектыСопровождения КАК ВтПроектыСопровождения
	        |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ТехническиеПроекты КАК ТехПроекты
	        |			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЭП_РеализацияУслуг КАК РеализацияУслуг
	        ...
	        |	ЭП_ПоступлениеУслуг.Ссылка КАК Акт,
	        |	ЭП_ПоступлениеУслуг.Сумма КАК СуммаАкта,
	        |	ВтЭтапыСопровождениеБезПост.Проект КАК Проект,
	        |	ВтЭтапыСопровождениеБезПост.Этап КАК Этап
	        |ПОМЕСТИТЬ ВтЭтапыСопровождение
	        |ИЗ
	        |	ВтЭтапыСопровождениеБезПост КАК ВтЭтапыСопровождениеБезПост
	        |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЭП_ПоступлениеУслуг.ПроектыСопровождения КАК ЭП_ПоступлениеУслугПроектыСопровождения
	        |			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЭП_ПоступлениеУслуг КАК ЭП_ПоступлениеУслуг
	        |			ПО ЭП_ПоступлениеУслугПроектыСопровождения.Ссылка = ЭП_ПоступлениеУслуг.Ссылка
	        |		ПО ВтЭтапыСопровождениеБезПост.ТехПроект = ЭП_ПоступлениеУслугПроектыСопровождения.ТехническийПроект
	        |ГДЕ
	        |	ЭП_ПоступлениеУслуг.Проведен
	        |;
	        |
	        |////////////////////////////////////////////////////////////////////////////////
	        |ВЫБРАТЬ
	        ...
	        |	ЭП_ПоступлениеУслуг.Сумма КАК СуммаАкта,
	        |	ВтПроектыСопровождения.Ссылка КАК Проект
	        |ПОМЕСТИТЬ ВтПроектСопрСНеопредЭтапом
	        |ИЗ
	        |	ВтПроектыСопровождения КАК ВтПроектыСопровождения
	        |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ТехническиеПроекты КАК ТехническиеПроекты
	        |			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЭП_РеализацияУслуг КАК РеализацияУслуг
	        |			ПО ТехническиеПроекты.ЭП_РеализацияУслуг = РеализацияУслуг.Ссылка
	        |			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЭП_ПоступлениеУслуг КАК ЭП_ПоступлениеУслуг
	        |				ПРАВОЕ СОЕДИНЕНИЕ Документ.ЭП_ПоступлениеУслуг.ПроектыСопровождения КАК ЭП_ПоступлениеУслугПроектыСопровождения
	        |				ПО (ЭП_ПоступлениеУслугПроектыСопровождения.Ссылка = ЭП_ПоступлениеУслуг.Ссылка)
	        |			ПО (ТехническиеПроекты.Ссылка = ЭП_ПоступлениеУслугПроектыСопровождения.ТехническийПроект)
	        |		ПО ВтПроектыСопровождения.Ссылка = ТехническиеПроекты.Владелец
	        |ГДЕ
	        |	РеализацияУслуг.Ссылка ЕСТЬ NULL
	        |	И ЭП_ПоступлениеУслуг.Проведен
	        |	И ВтПроектыСопровождения.Ссылка В
	        |			(ВЫБРАТЬ
	        |				Проекты.Проект
	        |			ИЗ
	        |				ВтЭтапыСопровождениеБезПост КАК Проекты)
	        |	И (ТехническиеПроекты.ДатаЗавершения МЕЖДУ &НачалоПериода И &КонецПериода
	        |				И ТехническиеПроекты.ЭП_Архивный
	        |			ИЛИ НЕ ТехническиеПроекты.ЭП_Архивный)
	        |;
	        |
	        |////////////////////////////////////////////////////////////////////////////////
	        |ВЫБРАТЬ
	        |	ВтЭтапыНеСопровождение.СуммаАкта КАК СуммаАкта,
	        |	ВтЭтапыНеСопровождение.Акт КАК Акт
	        ...
	        |
	        |ВЫБРАТЬ
	        |	ВтПроектСопрСНеопредЭтапом.СуммаАкта,
	        |	ВтПроектСопрСНеопредЭтапом.Акт
	        |ИЗ
	        |	ВтПроектСопрСНеопредЭтапом КАК ВтПроектСопрСНеопредЭтапом
	        |ГДЕ
	        |	(НЕ &Проект ЕСТЬ NULL
	        |				И &Проект = ВтПроектСопрСНеопредЭтапом.Проект
	        |			ИЛИ &Проект ЕСТЬ NULL)
	        |	И &Этап ЕСТЬ NULL
	        |;
	        |
	        |////////////////////////////////////////////////////////////////////////////////
	        |ВЫБРАТЬ РАЗЛИЧНЫЕ
	        |	ВтРезультат.Акт КАК Акт,
	        |	ВтРезультат.СуммаАкта КАК СуммаАкта
	        |ИЗ
	        |	ВтРезультат КАК ВтРезультат";	
КонецФункции

&НаСервереБезКонтекста
Функция ЗапросРасшифровкиТрудозатрат()

	Возврат "ВЫБРАТЬ
	        |	Пользователи.Ссылка КАК Пользователь
	        |ПОМЕСТИТЬ ВтПользователиБезГПХ
	        |ИЗ
	        |	Справочник.Пользователи КАК Пользователи
	        |ГДЕ
	        |	НЕ Пользователи.ЭП_Сотрудник.ДоговорГПХ
	        |;
	        |
	        |////////////////////////////////////////////////////////////////////////////////
	        |ВЫБРАТЬ
	        ...
	        |			ПО РазделыПроекта.Владелец = Проекты.Ссылка
	        |		ПО УчетВремениЗатратыВремени.Этап = РазделыПроекта.Ссылка
	        |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.УчетВремени КАК УчетВремени
	        |		ПО УчетВремениЗатратыВремени.Ссылка = УчетВремени.Ссылка
	        |ГДЕ
	        |	НЕ Проекты.ЭП_ПроектСопровождение
	        |	И НЕ Проекты.ПометкаУдаления
	        |	И (РазделыПроекта.ДатаОкончания МЕЖДУ &НачалоПериода И &КонецПериода
	        ...
	        |	ВтПроектыСопровождения.Ссылка КАК Проект,
	        |	РазделыПроекта.Ссылка КАК Этап
	        |ПОМЕСТИТЬ ВтВсеЭтапыСопровождение
	        |ИЗ
	        |	ВтПроектыСопровождения КАК ВтПроектыСопровождения
	        |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.РазделыПроекта КАК РазделыПроекта
	        |		ПО ВтПроектыСопровождения.Ссылка = РазделыПроекта.Владелец
	        ...
	        |				ПО РеализацияУслуг.Этап = Этапы.Ссылка
	        |			ПО ТехПроекты.ЭП_РеализацияУслуг = РеализацияУслуг.Ссылка
	        |			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.УчетВремени.ЗатратыВремени КАК УчетВремениЗатратыВремени
	        |				ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.УчетВремени КАК УчетВремени
	        |				ПО УчетВремениЗатратыВремени.Ссылка = УчетВремени.Ссылка
	        |			ПО (УчетВремениЗатратыВремени.ТехническийПроект = ТехПроекты.Ссылка)
	        |		ПО (ТехПроекты.Владелец = ВтПроектыСопровождения.Ссылка)
	        |ГДЕ
	        |	НЕ ТехПроекты.ПометкаУдаления
	        |	И НЕ Этапы.ПометкаУдаления
	        ...
	        |			ИЛИ НЕ Этапы.ЭП_Архивный
	        |				И Этапы.ДатаОкончания < &КонецПериода)
	        |	И УчетВремени.Проведен
	        |;
	        |
	        |////////////////////////////////////////////////////////////////////////////////
	        |ВЫБРАТЬ
	        |	ВтВсеЭтапыСопровождения.Проект КАК Проект,
	        |	ВтВсеЭтапыСопровождения.Этап КАК Этап,
	        |	ВтЭтапыСопровождение.ТехПроект КАК ТехПроект
	        |ПОМЕСТИТЬ ВтЭтапыСопровождениеДляИспользования
	        |ИЗ
	        |	ВтВсеЭтапыСопровождение КАК ВтВсеЭтапыСопровождения,
	        |	ВтЭтапыСопровождение КАК ВтЭтапыСопровождение
	        |;
	        |
	        |////////////////////////////////////////////////////////////////////////////////
	        ...
	        |			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЭП_РеализацияУслуг КАК РеализацияУслуг
	        |			ПО ТехническиеПроекты.ЭП_РеализацияУслуг = РеализацияУслуг.Ссылка
	        |			ПРАВОЕ СОЕДИНЕНИЕ Документ.УчетВремени.ЗатратыВремени КАК УчетВремениЗатратыВремени
	        |				ЛЕВОЕ СОЕДИНЕНИЕ Документ.УчетВремени КАК УчетВремени
	        |				ПО УчетВремениЗатратыВремени.Ссылка = УчетВремени.Ссылка
	        |			ПО (УчетВремениЗатратыВремени.ТехническийПроект = ТехническиеПроекты.Ссылка)
	        |		ПО ВтПроектыСопровождения.Ссылка = ТехническиеПроекты.Владелец
	        |ГДЕ
	        |	РеализацияУслуг.Ссылка ЕСТЬ NULL
	        |	И ВтПроектыСопровождения.Ссылка В
	        ...
	        |				Проекты.Проект
	        |			ИЗ
	        |				ВтЭтапыСопровождениеДляИспользования КАК Проекты)
	        |	И (ТехническиеПроекты.ПлановаяДатаОкончания МЕЖДУ &НачалоПериода И &КонецПериода
	        |				И ТехническиеПроекты.ЭП_Архивный
	        |			ИЛИ НЕ ТехническиеПроекты.ЭП_Архивный)
	        |	И УчетВремени.Проведен
	        |
	        |СГРУППИРОВАТЬ ПО
	        |	ВтПроектыСопровождения.Ссылка,
	        |	УчетВремени.Ссылка,
	        ...
	        |////////////////////////////////////////////////////////////////////////////////
	        |ВЫБРАТЬ
	        |	ВтЭтапыНеСопровождение.Ссылка КАК Ссылка,
	        |	ВтЭтапыНеСопровождение.Длительность КАК Длительность
	        |ПОМЕСТИТЬ ВтРезультат
	        |ИЗ
	        |	ВтЭтапыНеСопровождение КАК ВтЭтапыНеСопровождение
	        |ГДЕ
	        ...
	        |ВЫБРАТЬ
	        |	ВтЭтапыНеСопровождение.Ссылка,
	        |	ВтЭтапыНеСопровождение.Длительность
	        |ИЗ
	        |	ВтЭтапыНеСопровождение КАК ВтЭтапыНеСопровождение
	        |ГДЕ
	        |	НЕ &Итог
	        ...
	        |ВЫБРАТЬ
	        |	ВтЭтапыСопровождение.Ссылка,
	        |	ВтЭтапыСопровождение.Длительность
	        |ИЗ
	        |	ВтЭтапыСопровождение КАК ВтЭтапыСопровождение
	        |ГДЕ
	        |	&Итог
	        ...
	        |
	        |ВЫБРАТЬ
	        |	ВтПроектСопрСНеопредЭтапом.Ссылка,
	        |	ВтПроектСопрСНеопредЭтапом.Длительность
	        |ИЗ
	        |	ВтПроектСопрСНеопредЭтапом КАК ВтПроектСопрСНеопредЭтапом
	        |ГДЕ
	        |	(НЕ &Проект ЕСТЬ NULL
	        ...
	        |////////////////////////////////////////////////////////////////////////////////
	        |ВЫБРАТЬ
	        |	ВтРезультат.Ссылка КАК Акт,
	        |	ОКР(ВтРезультат.Длительность / 3600) КАК СуммаАкта
	        |ИЗ
	        |	ВтРезультат КАК ВтРезультат";
...
&НаСервереБезКонтекста
Функция ПолучитьРекурсивноСтруктуруОтбора(Расшифровка, Адрес, СтруктураОтбора = Неопределено, ДанныеРасшифровки = Неопределено)
	...
	Для каждого ТекЭл Из ТекущееПоле.ПолучитьРодителей() Цикл
		ПолучитьРекурсивноСтруктуруОтбора(ТекЭл.Идентификатор, Адрес, СтруктураОтбора, ДанныеРасшифровки);    
	КонецЦикла;
...
Функция ПолучитьЗначениеРасшифровки(Расшифровка, ИмяПоля)  
	...
	ЗначениеПолей = ДанныеРасшифровки.Элементы.Получить(Расшифровка).ПолучитьПоля();  
	Поле = ЗначениеПолей.Найти(ИмяПоля);
