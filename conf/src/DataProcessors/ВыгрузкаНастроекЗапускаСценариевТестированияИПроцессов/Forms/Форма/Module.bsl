...
&НаКлиенте
Процедура ОбновитьКешДанныхСценариевСлужебный(КаталогТестов, ИмяФайлаРезультата)
	...
	Для Каждого Файл Из Файлы Цикл
		...
		ДанныеСценария.Вставить("ДанныеДляУдаленияВызывающиеСцеанриии", Неопределено);
	...
	Для Каждого Элем Из КешДанныхСценариев Цикл
		...
		Для Каждого ДанныеВызывающегоСценария Из Элем.Значение.ВызывающиеСценарии Цикл
			...
			Если ДанныеСценарияИзКеш.ВложенныеСценарии[UIDСценарияДляОбработки] = Неопределено Тогда
				...
				Если Элем.Значение.ДанныеДляУдаленияВызывающиеСцеанриии = Неопределено Тогда
					Элем.Значение.ДанныеДляУдаленияВызывающиеСцеанриии = Новый Массив;
				КонецЕсли;	 
				...
				Элем.Значение.ДанныеДляУдаленияВызывающиеСцеанриии.Добавить(ДанныеВложенногоСценарияДляОбновления);
	...
	Для Каждого Элем Из КешДанныхСценариев Цикл
		...
		Если Элем.Значение.ДанныеДляУдаленияВызывающиеСцеанриии <> Неопределено Тогда
			Для Каждого ТекДанныеДляУдаленияВызывающиеСцеанриии Из Элем.Значение.ДанныеДляУдаленияВызывающиеСцеанриии Цикл
				Если ТипЗнч(ДанныеСценарияИзYaml["ВызывающиеСценарии"]) <> Тип("Массив") Тогда
					ДанныеСценарияИзYaml.Вставить("ВызывающиеСценарии", Новый Массив);
				КонецЕсли;	 
				...
				Для Каждого ТекВызывающийСценарий Из ДанныеСценарияИзYaml["ВызывающиеСценарии"] Цикл
					Ид = Ид + 1;
					Если ТекВызывающийСценарий["UIDВызывающийСценарий"] = ТекДанныеДляУдаленияВызывающиеСцеанриии.UIDВызывающийСценарий Тогда
		...
		Если НадоВыгрузитьЭлемент Тогда
...
&НаКлиенте
Процедура ОбновитьКешДанныхСценариев()
	КаталогВыгрузкиСценариев = ДополнитьРазделительПути(КаталогВыгрузкиСценариев);
	ОбновитьКешДанныхСценариевСлужебный(КаталогВыгрузкиСценариев, КаталогВыгрузкиСценариев + "scencache.json");
КонецПроцедуры 
