...
Процедура ПередЗагрузкойДанныхИзНастроекНаСервере(Настройки)
	...
	Если СохраненныеУчастникиВидаПлана <> Неопределено Тогда
		...
		УчастникиВидаПлана                                         = СохраненныеУчастникиВидаПлана;
...
Процедура ПланЗанятостиОтСотрудниковВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли (Поле.Имя = "ПланЗанятостиТехПроект"
		Или Поле.Имя = "ПланЗанятостиВерсия")
		И ЗначениеЗаполнено(ДанныеСтроки.ТехническийПроект) Тогда
...
Процедура ДекорацияОтборПоТехПроектамОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "ОткрытьПодборТехПроектов" Тогда
		...
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("ОтборТехническиеПроекты", ВыводимыеТехническиеПроекты);
...
&НаКлиенте
Процедура ДобавитьВОтборПоСотрудникамВыделенные(Команда)
...
&НаКлиенте
Процедура ДобавитьВОтборПоТехпроектамВыделенные(Команда)
...
Процедура РаспределитьНераспределено(Команда)
	...
	Если СтрокаСотрудник.Нераспределено = 0 Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(СтрШаблон(НСтр("ru = 'Для сотрудника %1 нет нераспределенных часов в плане занятости.'"),
		                                            СтрокаСотрудник.СотрудникРабота)); 
		Возврат;
...
Процедура ЗаполнитьДанныеВидаПлана(ПараметрыИзмененияВидаПлана)
	...
	Планирование.СформироватьПредставлениеОтображаемыхУчастниковПланаНаСервере(ЭтотОбъект, "ДекорацияПредставлениеОтборСотрудников", ПодразделенияСотрудники);
	ЗаполнитьПодборУчастниковДляОтображенияОтТехПроектов(ПодразделенияСотрудники);
	ПланированиеКлиентСервер.СформироватьПредставлениеОтображаемыхТехПроектов(ВыводимыеТехническиеПроекты, Элементы.ДекорацияОтборПоТехПроектам, ЦветТекстаГиперссылки);
...
Процедура ПослеЗакрытияВыбораУчастников(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ПланированиеКлиент.ЕстьИзмененияПослеВыбораУчастников(ЭтотОбъект, Результат, "ДекорацияПредставлениеОтборСотрудников") Тогда
		ОбновитьДанныеСтраницаОтСотрудниковНаКлиенте();
	КонецЕсли;
...
Процедура ИзменитьОтборСотрудникиПоКомандеИОбновить(ИмяКоманды)
	...
	Если ИмяКоманды = "ДобавитьВОтборПоСотрудникамВыделенные" Тогда
		
		Для Каждого ВыделенныйСотрудник Из ВыделенныеСотрудники Цикл
			
			НайденныйЭлемент = УчастникиВидаПлана.НайтиПоЗначению(ВыделенныйСотрудник);
	...
	ИначеЕсли ИмяКоманды = "УстановитьОтборПоВыделеннымСотрудникам" Тогда
			...
			Пока ВыборкаЗначениеУпорядочивания.Следующий() Цикл
				...
				Пока ВыборкаТехническийПроект.Следующий() Цикл
					...
					Пока ВыборкаРабота.Следующий() Цикл
						...
						СтрокаРабота.СотрудникРабота   = ?(ТипЗнч(ВыборкаРабота.Работа) = Тип("СправочникСсылка.ТехническиеПроекты"), 
						                                   Справочники.ЗадачиПроцесса.ПустаяСсылка(), 
						                                   ВыборкаРабота.Работа);
...
Процедура ЗаполнитьДеревоОтСотрудников()
	...
	ВыбранныеСотрудники = ВыбранныеДляОтображенияСотрудники(ЭтотОбъект);
...
Процедура ОпределитьДоступностьРедактированияПлановыеТрудозатратыДляСтроки(Форма, СтрокаРабота, Вариант)
	...
	Для Каждого СтрокаОтображаемыеПланы Из Форма.ОтображаемыеПланы Цикл
		...
		ИначеЕсли Вариант = "ОтСотрудников" 
			     ...
			     И СтрокаРабота.ТипСотрудникРабота <> Тип("СправочникСсылка.ЗадачиПроцесса") Тогда
...
&НаКлиентеНаСервереБезКонтекста
Функция ВыбранныеДляОтображенияСотрудники(Форма)
	
	ОтборСотрудников = Новый Массив;
	
	Для Каждого ЭлементСписка Из Форма.УчастникиВидаПлана Цикл
		Если ЭлементСписка.Пометка Тогда
			ОтборСотрудников.Добавить(ЭлементСписка.Значение);
		КонецЕсли;
	КонецЦикла;
	
	Возврат ОтборСотрудников;
...
Процедура ПослеЗакрытияОтбораТехническиеПроекты(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ПланированиеКлиентСервер.СпискиЗначенийИдентичны(Результат, ВыводимыеТехническиеПроекты) Тогда
		Возврат;
	КонецЕсли;
	
	ВыводимыеТехническиеПроекты = Результат;
...
Процедура ИзменитьОтборТехПроектыПоКомандеИОбновить(ИмяКоманды)
	...
	Если ИмяКоманды = "ДобавитьВОтборПоТехпроектамВыделенные" Тогда
		
		Для Каждого ВыделенныйТехПроект Из ВыделенныеТехПроектыПланОтСотрудников Цикл
			
			Если ВыводимыеТехническиеПроекты.НайтиПоЗначению(ВыделенныйТехПроект) = Неопределено Тогда
				ВыводимыеТехническиеПроекты.Добавить(ВыделенныйТехПроект);
			КонецЕсли;
			
		КонецЦикла;
		
	ИначеЕсли ИмяКоманды = "УстановитьОтборПоВыделеннымТехпроектам" Тогда
...
Процедура ЗаполнитьДеревоОтТехническихПроектов()
	...
	ЗадачиПроцессов.ПоказатьВременнуюТаблицу(Запрос, "СуществующиеЗаписиПлана");
			...
			Пока ВыборкаСотрудники.Следующий() Цикл
				...
				Пока ВыборкаДетали.Следующий() Цикл
					...
					Если Не ДополнительныеДанныеРаботЗаполнены Тогда
						...
						Если ЗначениеЗаполнено(СтрокаЭтап.СотрудникРабота) Тогда
							СтрокаСотрудник.ДатаНачала        = ВыборкаДетали.ПлановаяДатаНачалаЗадачи;
							СтрокаСотрудник.ДатаОкончания     = ВыборкаДетали.ПлановаяДатаОкончанияЗадачи; 
						Иначе
							СтрокаСотрудник.ДатаНачала        = ВыборкаДетали.ПлановаяДатаНачалаТехпроекта;
							СтрокаСотрудник.ДатаОкончания     = ВыборкаДетали.ПлановаяДатаОкончанияТехпроекта;
						КонецЕсли;
...
Процедура ПослеИзмененияПлановыхТрудозатратОтСотрудника(ТекущиеДанные, ИмяТекущейКолонки);
	...
	ПересчитатьНераспределено(ТекущиеДанные, Тип("СправочникСсылка.ЗадачиПроцесса"));
...
Процедура ПослеИзмененияПлановыхТрудозатратОтТехПроекта(ТекущиеДанные, ИмяТекущейКолонки) 
	...
	ПересчитатьНераспределено(ТекущиеДанные, Тип("СправочникСсылка.Пользователи"));
	ПересчитатьМассивКолонокРодительскихСтрокДереваПослеИзмененияПодчиненного(
		ЭтотОбъект,
		ТекущиеДанные, 
...
Функция ДанныеЗаписиРегистраПоТаблицеОтСотрудника(ТекущиеДанные, ИмяТекущейКолонки)
	...
	ДанныеЗаписиПлана.Работа                  = ?(ТекущиеДанные.ТипСотрудникРабота = Тип("СправочникСсылка.ЗадачиПроцесса") И Не ЗначениеЗаполнено(ТекущиеДанные.СотрудникРабота),
	                                              ТекущиеДанные.ТехническийПроект,
	                                              ТекущиеДанные.СотрудникРабота);
	ДанныеЗаписиПлана.ТипСтрокиПлана          = ПредопределенноеЗначение("Перечисление.ТипыСтрокПлана.Работа");
	ДанныеЗаписиПлана.ПланируемыеТрудозатраты = ТекущиеДанные[ИмяТекущейКолонки];
...
Процедура ОтразитьИзменениеПлановыхТрудозатратНаДругойСтранице(ИмяИзменяемогоДерева, ИзменяемыеЗаписи)
	...
	Для Каждого ИзменяемаяЗапись Из ИзменяемыеЗаписи Цикл
		...
		Если ИмяИзменяемогоДерева = "ПланЗанятостиОтТехПроектов" Тогда
			...
			ДоступныйТип                    = Тип("СправочникСсылка.ЗадачиПроцесса");
		...
		ИначеЕсли ИмяИзменяемогоДерева = "ПланЗанятостиОтСотрудников" Тогда
			...
			ДоступныйТип                    = Тип("СправочникСсылка.Пользователи");
...
Процедура ВывестиДополнительныеДанныеОтТехПроектов(АдресРезультата, ДополнительныеПараметры)
	...
	Для Каждого ЭтапСотрудники Из ДополнительныеДанные.ЭтапыСотрудники Цикл 
		...
		Для Каждого ДанныеСотрудника Из ЭтапСотрудники.СотрудникиЭтапа Цикл
			...
			ПересчитатьНераспределено(СтрокаСотрудник, Тип("СправочникСсылка.Пользователи"));
...
Процедура ВывестиДополнительныеДанныеОтСотрудников(АдресРезультата, ДополнительныеПараметры)
	...
	Для Каждого СотрудникЭтапы Из ДополнительныеДанные.СотрудникЭтапы Цикл
		...
		Для Каждого ДанныеЭтапа Из СотрудникЭтапы.ЭтапыСотрудника Цикл
			...
			ПересчитатьНераспределено(СтрокаЭтап, Тип("СправочникСсылка.ЗадачиПроцесса"));
...
&НаКлиенте
Процедура ПересчитатьНераспределено(СтрокаДерева, ДопустимыйТип)
	
	Если СтрокаДерева.ТипСотрудникРабота <> ДопустимыйТип Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОткрытьФормуЗадачЭтапаСотрудника(ПараметрыОткрытия)
	...
	Если ЗначениеЗаполнено(ПараметрыОткрытия.Этап) Тогда
...
Процедура ПослеЗакрытияФормыНастроекРаспределения(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Не РезультатПроверкиНастроекРаспределения(Результат) Тогда
		Возврат;
	КонецЕсли;
	
	ВыполнитьРаспределениеПоУказаннымНастройкам(Результат);
...
Функция РезультатПроверкиНастроекРаспределения(НастройкиРаспределения)
	...
	Для Каждого ЭлементНастроек Из НастройкиРаспределения.Этапы Цикл
		
		Если ЭлементНастроек.Нераспределено > 0 Тогда
			Возврат Истина;
		КонецЕсли;
...
Процедура РаспределениеПоНастройкамЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Выполнено" Тогда
		ОбработатьЗаверешениеРаспределенияПоНастройкам(Результат.АдресРезультата, ДополнительныеПараметры);
	КонецЕсли;
	
КонецПроцедуры 

&НаКлиенте
Процедура ОбработатьЗаверешениеРаспределенияПоНастройкам(АдресРезультата, ДополнительныеПараметры)
	...
	Иначе
		...
		ТекстЗаголовка = НСтр("ru = 'Распределение трудозатрат'");
		ТекстОповещения = СтрШаблон(НСтр("ru = 'Трудозатраты по участнику %1 успешно расределены'"), ДополнительныеПараметры.Сотрудник); 
