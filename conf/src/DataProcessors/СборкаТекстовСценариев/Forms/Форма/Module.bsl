...
Процедура СборкаСценариевИПроцессовСервер(ДанныеКаталогаПроцессов,ДанныеКаталогаСценариев,ДанныеЭталонныхБД)
	...
	КоличествоНастроекВПоследнемПотоке = КоличествоНастроек - КоличествоНастроекВОтборе * (Объект.КоличествоПотоков-1);
	Если КоличествоНастроекВПоследнемПотоке > Объект.КоличествоПотоков И КоличествоНастроекВПоследнемПотоке > КоличествоНастроекВОтборе Тогда
        КоличествоНастроекВОтборе = КоличествоНастроекВОтборе + 1;
    КонецЕсли;
	...
	ПраваяГраница = ?(Объект.НомерПотока < Объект.КоличествоПотоков, (Объект.НомерПотока) * КоличествоНастроекВОтборе, КоличествоНастроек);
