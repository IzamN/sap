...
Функция СформироватьДанныеПроекта(КаталогВыгрузки)
	...
	ЗаписьXML.ЗаписатьСоответствиеПространстваИмен("v8",  "http://v8.1c.ru/data");
		...
		ЗаписьXML.ЗаписатьНачалоЭлемента("ProjectData");
	    	...
	    	ЗаписатьДанныеПроекта(ЗаписьXML);
...
&НаСервере
Процедура ЗаписатьДанныеПроекта(ЗаписьXML)
	...
	Для Каждого ВидОбъекта Из ВидыОбъектов Цикл
		Для Каждого ОбъектМетаданных из Метаданные[ВидОбъекта] Цикл
			...
			ЗаписатьДанныеСсылочногоТипа(ОбъектМетаданных, ПолноеИмяОбъектаМетаданных, ЗаписьXML, ЭтоОбщийОбъект);
	...
	Для Каждого ОбъектМетаданных из Метаданные.РегистрыСведений Цикл
		...
		ЗаписатьДанныеНезависимыхРегистровСведений(ОбъектМетаданных, ЗаписьXML);
...
&НаСервере
Процедура ЗаписатьДанныеСсылочногоТипа(ОбъектМетаданных, ПолноеИмяОбъектаМетаданных, ЗаписьXML, ЭтоОбщийОбъект)
	...
	Если НЕ ЭтоОбщийОбъект
		...
		Запрос.УстановитьПараметр("Проект", Проект);
		УстановитьДополнительныеПараметрыЗапроса(Запрос, ПолноеИмяОбъектаМетаданных);
	КонецЕсли;
...
Функция УсловиеОтбораСсылочныхДанныхПоПроекту(ПолноеИмяОбъектаМетаданных)
	...
	Если ПолноеИмяОбъектаМетаданных = "Справочник.WebСервисыСвойства"
...
&НаСервере
Процедура УстановитьДополнительныеПараметрыЗапроса(Запрос, ПолноеИмяОбъектаМетаданных)
...
&НаСервере
Процедура ЗаписатьДанныеНезависимыхРегистровСведений(ОбъектМетаданных, ЗаписьXML)
...
Процедура ОбработатьДанныеОбъектаПередЗаписьюВФайл(ОбъектВыгрузки, ПолноеИмяОбъектаМетаданных)
	...
	ИначеЕсли ПолноеИмяОбъектаМетаданных = "Справочник.ТехническиеПроекты" Тогда
		...
		Если НЕ ОбъектВыгрузки.ЭтоГруппа Тогда
			...
			ОбъектВыгрузки.УдалитьКонтрольныеТочки.Очистить();
	...
	ОчиститьТехническийПроектИВеткуИсточник(ОбъектВыгрузки, ПолноеИмяОбъектаМетаданных);
...
&НаСервере
Процедура ОчиститьТехническийПроектИВеткуИсточник(ОбъектВыгрузки, ПолноеИмяОбъектаМетаданных)
	...
	Если (ПолноеИмяОбъектаМетаданных = "Справочник.КомандыОбъектовМетаданных" 
		...
		Если (НЕ ВыгружатьХарактеристикиПроекта ИЛИ НЕ ВыгружатьТехническиеПроекты)
			И ЗначениеЗаполнено(ОбъектВыгрузки.УдалитьТехническийПроектИсточник) Тогда
		
			ОбъектВыгрузки.УдалитьТехническийПроектИсточник = Справочники.ТехническиеПроекты.ПустаяСсылка();
...
Функция МассивОчищаемыхПолей(ИмяОбъекта)
	...
	ИначеЕсли ИмяОбъекта = "Справочник.Ошибки" Тогда
		...
		Массив.Добавить("УдалитьИзмененныеМетаданные.Комментарий");
