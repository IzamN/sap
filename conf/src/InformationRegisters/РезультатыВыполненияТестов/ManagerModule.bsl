...
Процедура ЗарегистрироватьДанныеКОбработкеДляЗаполненияПроекта(Параметры) Экспорт
	...
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	РезультатыВыполненияТестов.Ветка КАК Ветка
	|ИЗ
	|	РегистрСведений.РезультатыВыполненияТестов КАК РезультатыВыполненияТестов
	|ГДЕ
	|	РезультатыВыполненияТестов.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)";
	
	Результат = Запрос.Выполнить().Выгрузить();
	
	ДополнительныеПараметры = ОбновлениеИнформационнойБазы.ДополнительныеПараметрыОтметкиОбработки();
	ДополнительныеПараметры.ЭтоНезависимыйРегистрСведений = Истина;
	ДополнительныеПараметры.ПолноеИмяРегистра = "РегистрСведений.РезультатыВыполненияТестов";
...
Процедура ОбработатьДанныеДляЗаполненияПроекта(Параметры) Экспорт
	...
	МетаданныеОбъекта = Метаданные.РегистрыСведений.РезультатыВыполненияТестов;
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Предупреждение,
				Метаданные.РегистрыСведений.РезультатыВыполненияТестов, Выборка.Ветка, ТекстСообщения);
		КонецПопытки;
