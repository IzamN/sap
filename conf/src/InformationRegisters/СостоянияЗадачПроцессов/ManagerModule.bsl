...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос.Текст ="
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	СостоянияЗадачПроцессов.ЗадачаПроцесса КАК Ссылка
	|ИЗ
	|	РегистрСведений.СостоянияЗадачПроцессов КАК СостоянияЗадачПроцессов
	|ГДЕ
	|	СостоянияЗадачПроцессов.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияЗадачПроцессов.ПустаяСсылка)";
	...
	ОбновлениеИнформационнойБазы.ОтметитьКОбработке(Параметры, Результат.Выгрузить().ВыгрузитьКолонку("Ссылка"));
	
КонецПроцедуры

Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	
	ВыборкаСсылокДляОбработок = ОбновлениеИнформационнойБазы.ВыбратьСсылкиДляОбработки(Параметры.Очередь, "Справочник.ЗадачиПроцесса");
	...
	Пока ВыборкаСсылокДляОбработок.Следующий() Цикл
		МассивЗадачДляОбработки.Добавить(ВыборкаСсылокДляОбработок.Ссылка);
	КонецЦикла;
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	ЗадачиПроцесса.Ссылка                                                                                                    КАК Ссылка,
	|	ЕСТЬNULL(СостоянияЗадачПроцессов.МаксимальныйВозможныйСтатус, ЗНАЧЕНИЕ(Перечисление.СтатусыЗадачПроцессов.ПустаяСсылка)) КАК МаксимальныйВозможныйСтатус,
	|	ЕСТЬNULL(СостоянияЗадачПроцессов.Состояние, ЗНАЧЕНИЕ(Перечисление.СостоянияЗадачПроцессов.ПустаяСсылка))                 КАК Состояние,
	|	ЗадачиПроцесса.Статус                                                                                                    КАК Статус,
	|	ЗадачиПроцесса.ПометкаУдаления                                                                                           КАК ПометкаУдаления,
	|	ЕСТЬNULL(СостоянияЗадачПроцессов.ДатаГотовностиКВыполнению, ДАТАВРЕМЯ(1, 1, 1, 1, 1, 1))                                 КАК ДатаГотовностиКВыполнению
	|ИЗ
	|	РегистрСведений.СостоянияЗадачПроцессов КАК СостоянияЗадачПроцессов
	...
	Запрос.УстановитьПараметр("МассивЗадач", МассивЗадачДляОбработки);
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если Не ЗначениеЗаполнено(Выборка.Состояние)
				И Не Выборка.ПометкаУдаления Тогда
				
				ДатаГотовностиКВыполнению = Неопределено;
				Состояние = Перечисления.СостоянияЗадачПроцессов.СостояниеПоСтатусуИМаксимальноВозможномуСтатусу(Выборка.Статус, Выборка.МаксимальныйВозможныйСтатус);
				Если СостоянияГотовоКВыполнениюВыполняется.Найти(Состояние) <> Неопределено Тогда
					ДатаГотовностиКВыполнению =  ДатаГотовностиВыполненияЗадачи(Выборка.Ссылка);
				КонецЕсли;
				ЗаписатьНовыйМаксимальноВозможныйСтатус(Выборка.Ссылка, Выборка.МаксимальныйВозможныйСтатус, Состояние, ДатаГотовностиКВыполнению); 
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось обработать состояние задачи %1 по причине:
					|%2'"), 
					Выборка.Ссылка, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
					ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Предупреждение,
					Метаданные.РегистрыСведений.СостоянияЗадачПроцессов, , ТекстСообщения);
		КонецПопытки;
	
	КонецЦикла;
	
	Если ПроблемныхОбъектов <> 0 Тогда
		ТекстСообщения = НСтр("ru = 'Процедура ОбновитьСостоянияЗадачПроцессов завершилась с ошибкой. Не все записи состояний удалось обновить.'");
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
	
	Параметры.ОбработкаЗавершена = ОбновлениеИнформационнойБазы.ОбработкаДанныхЗавершена(Параметры.Очередь, "Справочник.ЗадачиПроцесса");
