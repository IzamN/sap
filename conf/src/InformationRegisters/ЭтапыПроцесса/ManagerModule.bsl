...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ЭтапыПроцесса КАК ЭтапыПроцесса
	|		ПО ТехническиеПроекты.Ссылка = ЭтапыПроцесса.Предмет
	|			И (ЭтапыПроцесса.ТекущийЭтапПроцесса <> ЗНАЧЕНИЕ(Справочник.ЗадачиШаблонаПроцесса.ПустаяСсылка))
	|			И (ЭтапыПроцесса.ЗадачаПроцесса = ЗНАЧЕНИЕ(Справочник.ЗадачиШаблонаПроцесса.ПустаяСсылка))
	|			И (ЭтапыПроцесса.МаксимальнаяДатаОкончанияЭтапов = ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0))";
	
	Результат = Запрос.Выполнить().Выгрузить();
	
	ДополнительныеПараметры = ОбновлениеИнформационнойБазы.ДополнительныеПараметрыОтметкиОбработки();
	ДополнительныеПараметры.ЭтоНезависимыйРегистрСведений = Истина;
	ДополнительныеПараметры.ПолноеИмяРегистра = "РегистрСведений.ЭтапыПроцесса";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПолноеИмяОбъекта = "РегистрСведений.ЭтапыПроцесса";
	ДополнительныеПараметры = ОбновлениеИнформационнойБазы.ДополнительныеПараметрыВыборкиДанныхДляОбработки();
	ДополнительныеПараметры.ИмяИзмеренияДляОтбора = "Предмет";
	...
	МассивПредметовКОбработке = Новый Массив;
	
	Пока Выборка.Следующий() Цикл 
		
		МассивПредметовКОбработке.Добавить(Выборка.Предмет);
	...
	|	МАКСИМУМ(ЗадачиПроцесса.КрайняяДатаОкончания) КАК МаксимальнаяДатаОкончанияЭтапов
	|ПОМЕСТИТЬ ТехПроектыМаксимальныеДатыЭтапов
	|ИЗ
	|	Справочник.ЗадачиПроцесса КАК ЗадачиПроцесса
	|ГДЕ
	...
	|	ЕСТЬNULL(ТехПроектыМаксимальныеДатыЭтапов.МаксимальнаяДатаОкончанияЭтапов, ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)) КАК МаксимальнаяДатаОкончанияЭтапов,
	|	ЕСТЬNULL(ЭтапыПроцесса.ШаблонПроцесса, ЗНАЧЕНИЕ(Справочник.ЗадачиШаблонаПроцесса.ПустаяСсылка))         КАК ШаблонПроцесса
	|ИЗ
	|	Справочник.ТехническиеПроекты КАК ТехническиеПроекты
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТехПроектыМаксимальныеДатыЭтапов КАК ТехПроектыМаксимальныеДатыЭтапов
	|		ПО ТехническиеПроекты.Ссылка = ТехПроектыМаксимальныеДатыЭтапов.Предмет
	...
	Запрос.УстановитьПараметр("ТехническиеПроекты", МассивПредметовКОбработке);
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			ЭлементБлокировкиДанных.УстановитьЗначение("Предмет",        Выборка.ТехническийПроект);
			ЭлементБлокировкиДанных.УстановитьЗначение("ШаблонПроцесса", Выборка.ШаблонПроцесса);
			ЭлементБлокировкиДанных.УстановитьЗначение("ЗадачаПроцесса", Справочники.ЗадачиШаблонаПроцесса.ПустаяСсылка());
			ЭлементБлокировкиДанных.Режим = РежимБлокировкиДанных.Исключительный;
			БлокировкаДанных.Заблокировать();
			
			НаборЗаписей = РегистрыСведений.ЭтапыПроцесса.СоздатьНаборЗаписей();
			НаборЗаписей.Отбор.Предмет.Установить(Выборка.ТехническийПроект);
			НаборЗаписей.Отбор.ШаблонПроцесса.Установить(Выборка.ШаблонПроцесса);
			НаборЗаписей.Отбор.ЗадачаПроцесса.Установить(Справочники.ЗадачиШаблонаПроцесса.ПустаяСсылка());
			НаборЗаписей.Прочитать();
			
			Если НаборЗаписей.Количество() = 1 Тогда
				
				ЗаписьРегистра = НаборЗаписей[0];
				ЗаписьРегистра.МаксимальнаяДатаОкончанияЭтапов = Выборка.МаксимальнаяДатаОкончанияЭтапов;
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось обработать запись по предмету: %1 по причине:
					|%2'"), 
					Выборка.ТехническийПроект, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			
			ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Предупреждение,
				Метаданные.РегистрыСведений.ЭтапыПроцесса, , ТекстСообщения);
	...
	Если ПроблемныхЗаписей <> 0 Тогда
		ТекстСообщения = НСтр("ru = 'Процедура по определению максимальной даты для этапов технических проектов завершилась с ошибкой. Не все данные удалось обновить.'");
