...
// Получает период последнего плана, в котором есть записи по сотруднику
//
// Параметры:
//  ВидПлана      - СправочникСсылка.ВидыПлана     - вид плана, по которому получаются данные.
//  Сотрудник     - СправочникСсылка.Пользователи - участник планирования, по которому получаются данные.
...
//     * ВыбранаКПереносу - Булево
//
Функция ПериодПоследнегоПланаСЗаписьюПоСотруднику( ВидПлана, Сотрудник, НачалоПериода) Экспорт
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ГДЕ
	|	ЗаписиПлана.Работа ССЫЛКА Справочник.ЗадачиПроцесса
	|	И ЗаписиПлана.Работа.ЗадачаШаблона = ЗНАЧЕНИЕ(Справочник.ЗадачиШаблонаПроцесса.ПустаяСсылка)";
	
	Результат = Запрос.Выполнить().Выгрузить();
	
	ДополнительныеПараметры = ОбновлениеИнформационнойБазы.ДополнительныеПараметрыОтметкиОбработки();
	ДополнительныеПараметры.ЭтоНезависимыйРегистрСведений = Истина;
	ДополнительныеПараметры.ПолноеИмяРегистра = "РегистрСведений.Планы";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПолноеИмяОбъекта = "РегистрСведений.Планы";
	...
	МассивИдентификаторовКОбработке = Новый Массив;
	
	Пока Выборка.Следующий() Цикл 
		
		МассивИдентификаторовКОбработке.Добавить(Выборка.ИдентификаторПлана);
	...
	|	ЗаписиПлана.Работа.Предмет          КАК ТехПроект,
	...
	|	ЗаписиПлана.Работа ССЫЛКА Справочник.ЗадачиПроцесса
	|	И ЗаписиПлана.Работа.ЗадачаШаблона = ЗНАЧЕНИЕ(Справочник.ЗадачиШаблонаПроцесса.ПустаяСсылка)
	|	И Планы.ИдентификаторПлана В(&МассивИдентификаторовКОбработке)
	|ИТОГИ ПО
	|	ИдентификаторПлана,
	|	Сотрудник,
	|	ТехПроект,
	...
	Запрос.УстановитьПараметр("МассивИдентификаторовКОбработке", МассивИдентификаторовКОбработке);
	
	ВыборкаИдентификаторы = Запрос.Выполнить().Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	
	ИдентификаторПлана = Неопределено;
	Сотрудник          = Неопределено;
	ТехПроект          = Неопределено;
	Работа             = Неопределено;
	ТипСтрокиПлана     = Перечисления.ТипыСтрокПлана.Работа;
	Группировка        = Неопределено;
	
	Пока ВыборкаИдентификаторы.Следующий() Цикл 
		
		ИдентификаторПлана = ВыборкаИдентификаторы.ИдентификаторПлана;
		
		ВыборкаСотрудники = ВыборкаИдентификаторы.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		
		Пока ВыборкаСотрудники.Следующий() Цикл
			
			Сотрудник = ВыборкаСотрудники.Сотрудник;
			
			ВыборкаТехПроекты = ВыборкаСотрудники.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
			
			Пока ВыборкаТехПроекты.Следующий() Цикл
				
				ТехПроект = ВыборкаТехПроекты.ТехПроект;
				ПланируемыеТрудозатраты = 0;
				
				НачатьТранзакцию();
				
				УстановитьБлокировкуПоПлану(ИдентификаторПлана);
				
				Попытка
					
					ВыборкаРаботы = ВыборкаТехПроекты.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
					
					Пока ВыборкаРаботы.Следующий() Цикл
						
						Работа = ВыборкаРаботы.Работа;
						
						ВыборкаДетали = ВыборкаРаботы.Выбрать();
						
						Пока ВыборкаДетали.Следующий() Цикл
							
							ПланируемыеТрудозатраты = ПланируемыеТрудозатраты + ВыборкаДетали.ПланируемыеТрудозатраты;
							
						КонецЦикла;
						
						НаборЗаписей = НаборЗаписейПоЗаписиПлана(ИдентификаторПлана, Сотрудник, Работа, ТипСтрокиПлана, Группировка);
						
						НаборЗаписей.Записать();
						
					КонецЦикла;
					
					НаборЗаписей = НаборЗаписейПоЗаписиПлана(ИдентификаторПлана, Сотрудник, ТехПроект, ТипСтрокиПлана, Группировка);
					ЗаписьНабора = НаборЗаписей.Добавить();
					ЗаписьНабора.ИдентификаторПлана      = ИдентификаторПлана;
					ЗаписьНабора.Сотрудник               = Сотрудник;
					ЗаписьНабора.Работа                  = ТехПроект;
					ЗаписьНабора.ТипСтрокиПлана          = ТипСтрокиПлана;
					ЗаписьНабора.Группировка             = Группировка;
					ЗаписьНабора.ПланируемыеТрудозатраты = ПланируемыеТрудозатраты;
...
Функция НаборЗаписейПоЗаписиПлана(ИдентификаторПлана, Сотрудник, Работа, ТипСтрокиПлана, Группировка)
	
	НаборЗаписей = РегистрыСведений.ЗаписиПлана.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.ИдентификаторПлана.Установить(ИдентификаторПлана);
	НаборЗаписей.Отбор.Сотрудник.Установить(Сотрудник);
	НаборЗаписей.Отбор.Работа.Установить(Работа);
	НаборЗаписей.Отбор.ТипСтрокиПлана.Установить(ТипСтрокиПлана);
	НаборЗаписей.Отбор.Группировка.Установить(Группировка);
	
	Возврат НаборЗаписей;
	
КонецФункции

Процедура УстановитьБлокировкуПоПлану(ИдентификаторПлана)
	
	БлокировкаДанных = Новый БлокировкаДанных;
	ЭлементБлокировкиДанных = БлокировкаДанных.Добавить("РегистрСведений.Планы");
	ЭлементБлокировкиДанных.УстановитьЗначение("ИдентификаторПлана", ИдентификаторПлана);
