...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОтключитьАвтоматическоеЗаполнениеПолейПриВыбореОписанияРаботыИзСписка = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
		"УчетВремени",
	...
	Если Пользователи.РолиДоступны("ЭП_ИспользованиеЭП_НастройкиУведомленийПоЗаявкам") Тогда
		МенеджерЗаписи = РегистрыСведений.ЭП_НастройкаУведомленийПоЗаявкам.СоздатьМенеджерЗаписи();
		МенеджерЗаписи.Пользователь = АвторизованныйПользователь;
		МенеджерЗаписи.Прочитать();
		УведомленияПоЗаявкам = МенеджерЗаписи.УведомленияПоЗаявкам;
	КонецЕсли;
	...
	Если Пользователи.РолиДоступны("ЭП_НастройкиУведомленийОбОтсутсвииОценокУСотрудников") Тогда
		...
		МенеджерЗаписи.Пользователь = АвторизованныйПользователь;
		МенеджерЗаписи.Прочитать();
		ЭП_УведомлятьОбОтсутствииОценокУСотрудников = МенеджерЗаписи.УведомленияПоОценкам;
	КонецЕсли;
...
Процедура ЗаписатьИЗакрыть(Команда)
	...
	КонецЕсли; 
	 ...
	 //Уведомления по заявкам
	УстановитьЗначениеУведомленияПоЗаявкам();
	УстановитьЗначениеУведомленияПоОценкамСотрудников();

	УведомленияПоЗаявкам();
	
КонецПроцедуры

&НаСервере
Процедура УстановитьЗначениеУведомленияПоОценкамСотрудников() 
	...
	Если Пользователи.РолиДоступны("ЭП_НастройкиУведомленийОбОтсутсвииОценокУСотрудников") Тогда
		...
		Если не МенеджерЗаписи.Выбран() Тогда
		    МенеджерЗаписи.Пользователь = АвторизованныйПользователь;
			МенеджерЗаписи.УведомленияПоОценкам = УведомлятьОбОтсутствииОценокУСотрудников;
			МенеджерЗаписи.Записать();
		ИначеЕсли МенеджерЗаписи.УведомленияПоОценкам <> УведомлятьОбОтсутствииОценокУСотрудников Тогда
			МенеджерЗаписи.УведомленияПоОценкам = УведомлятьОбОтсутствииОценокУСотрудников;
			МенеджерЗаписи.Записать();
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Процедура УстановитьЗначениеУведомленияПоЗаявкам() 
	...
	Если Пользователи.РолиДоступны("ЭП_ИспользованиеЭП_НастройкиУведомленийПоЗаявкам") Тогда
		...
		Если не МенеджерЗаписи.Выбран() Тогда
		    МенеджерЗаписи.Пользователь = АвторизованныйПользователь;
			МенеджерЗаписи.УведомленияПоЗаявкам = УведомленияПоЗаявкам;
			МенеджерЗаписи.Записать();
		ИначеЕсли МенеджерЗаписи.УведомленияПоЗаявкам <> УведомленияПоЗаявкам Тогда
			МенеджерЗаписи.УведомленияПоЗаявкам = УведомленияПоЗаявкам;
			МенеджерЗаписи.Записать();
		КонецЕсли;
	КонецЕсли;
