...
//
Процедура СформироватьERДиаграммуОбъекта(Ссылка, Документ, НастройкиДиаграммы) Экспорт
	...
	Выборка = ВыполнитьЗапросДляОбъектаМетаДанных(Ссылка, НастройкиДиаграммы).Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
...
Функция ВыполнитьЗапросДляОбъектаМетаданных(Ссылка, НастройкиДиаграммы)
	...
	|		И НЕ ОбъектыМетаданных.ЭП_НеПоказыватьНаДиаграмме
	|		И &ПоказыватьИсходящиеСвязи
	|	
	|	ОБЪЕДИНИТЬ
	|	
	|	ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|		КОГДА РеквизитыОбъектовМетаданныхТипыЗначенияРеквизита.ТипЗначенияРеквизита = &ОбъектМетаданных
	|				И &ПоказыватьИсходящиеСвязи
	|			ТОГДА РеквизитыОбъектовМетаданныхТипыЗначенияРеквизита.ТипЗначенияРеквизита
	|		ИНАЧЕ NULL
	|	КОНЕЦ КАК ТипЗначенияРеквизита
	|ПОМЕСТИТЬ РеквизитыОбъектовМетаданных
	|ИЗ
	...
	|				ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаОбъектовМетаданных КАК ОтборТиповОбъектов
	|				ПО РеквизитыОбъектовМетаданныхТипыЗначенияРеквизита.ТипЗначенияРеквизита = ОтборТиповОбъектов.Объект
	|			ПО РеквизитыОбъектовМетаданных.Ссылка = РеквизитыОбъектовМетаданныхТипыЗначенияРеквизита.Ссылка
	|				И (НЕ РеквизитыОбъектовМетаданныхТипыЗначенияРеквизита.Ссылка.ИгнорироватьСсылкиИзРеквизита)
	|				И (НЕ РеквизитыОбъектовМетаданныхТипыЗначенияРеквизита.Ссылка.ПометкаУдаления)
	...
	|	РеквизитыОбъектовМетаданных.Реквизит КАК Реквизит,
	|	РеквизитыОбъектовМетаданных.Реквизит.Имя КАК ИмяРеквизита,
	|	РеквизитыОбъектовМетаданных.Реквизит.Синоним КАК СинонимРеквизита,
	|	РеквизитыОбъектовМетаданных.ТипЗначенияРеквизита КАК ТипЗначенияРеквизита,
	|	РеквизитыОбъектовМетаданных.ТипРеквизита КАК ТипРеквизита,
	|	РеквизитыОбъектовМетаданных.ТипРеквизита.Имя КАК ТипРеквизитаИмя,
	|	РеквизитыОбъектовМетаданных.ТипРеквизита.Синоним КАК ТипРеквизитаСиноним,
	|	РеквизитыОбъектовМетаданных.ТипРеквизита.ВидРеквизита КАК ТипРеквизитаВидРеквизита,
	|	ВЫБОР
	|		КОГДА РеквизитыОбъектовМетаданных.ТипРеквизита.ВидРеквизита = ЗНАЧЕНИЕ(Перечисление.ВидыРеквизитов.ТабличнаяЧасть)
	|			ТОГДА 1
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК ПризнакТабличнойЧасти
	|ИЗ
	|	ТаблицаОбъектовМетаданных КАК ТаблицаОбъектовМетаданных
	|		ЛЕВОЕ СОЕДИНЕНИЕ РеквизитыОбъектовМетаданных КАК РеквизитыОбъектовМетаданных
	|		ПО ТаблицаОбъектовМетаданных.Объект = РеквизитыОбъектовМетаданных.Объект
	|			И ТаблицаОбъектовМетаданных.ОсновнойЭлемент = РеквизитыОбъектовМетаданных.ОсновнойЭлемент
	|			И (НЕ &ТолькоСвязанныеСвойства
	|				ИЛИ РеквизитыОбъектовМетаданных.ТипЗначенияРеквизита ЕСТЬ НЕ NULL )
	|
	|УПОРЯДОЧИТЬ ПО
	|	ТипСсылки УБЫВ,
	|	ОсновнойЭлемент,
	...
	Запрос.УстановитьПараметр("ТолькоСвязанныеСвойства", НастройкиДиаграммы.ТолькоСвязанныеСвойства);
	Запрос.УстановитьПараметр("ПоказыватьВходящиеСвязи", НастройкиДиаграммы.ПоказыватьСвязи = "Все" Или НастройкиДиаграммы.ПоказыватьСвязи = "Входящие");
	Запрос.УстановитьПараметр("ПоказыватьИсходящиеСвязи", НастройкиДиаграммы.ПоказыватьСвязи = "Все" Или НастройкиДиаграммы.ПоказыватьСвязи = "Исходящие");
	РезультатЗапроса = Запрос.ВыполнитьПакет();
	Возврат РезультатЗапроса[2];
