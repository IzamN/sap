...
Процедура ОбновитьСостояниеТестированияВеток(Ветки) Экспорт
	...
	Для Каждого ДанныеВетки Из ТекущееСостояниеТестированияВеток Цикл
		...
		ДанныеПоЗаблокированнымТестамВВетке = ДанныеПоЗаблокированнымТестамВВетке(ДанныеВетки.Ссылка, ДопПараметры);
		
		ГотоваКПомещению = ДопПараметры.ГотоваКПомещению;
		ТребуетсяРешение = ДопПараметры.ТребуетсяРешение; 
...
Функция ИнформацияОЗаданияхGitLab(Соединение, Токен, Проект, НомерЗапускТестированияGitLab) Экспорт
	...
	Пока Истина Цикл
		Ресурс = СтрШаблон("/api/v4/projects/%1/pipelines/%2/jobs/?page=%3&per_page=100", 
			СтрЗаменить(Проект, "/", "%2F"), Формат(НомерЗапускТестированияGitLab, "ЧГ="), Сч);
...
Процедура ОбработатьДанныеЗапускаТестированияGitLab(Pipe, ДанныеПроекта, Соединение, ЗагруженныеВетки=Неопределено)
	...
	Для Каждого ЗаданиеGitLab Из ВсеЗаданияGitLab Цикл
		Если ЗаданиеGitLab.duration <> Неопределено Тогда
			Длительность = Длительность + ЗаданиеGitLab.duration;
		КонецЕсли;	
	...
	ПараметрыЗапускаТестирования.Вставить("ref", НомерЗапускТестированияGitLab);
	...
	ЗапускТестирования = НовыйЗапускТестирования(ПараметрыЗапускаТестирования);
	...
	Для Каждого ЗаданиеGitLab Из ЗаданияGitLab Цикл
		...
		Если (СтрНайти(НРег(ЗаданиеGitLab.name),"bdd_test") > 0) 
			ИЛИ (СтрНайти(НРег(ЗаданиеGitLab.name),"bdd test") > 0)
			ИЛИ (СтрНайти(НРег(ЗаданиеGitLab.name),"bdd windows") > 0)
			ИЛИ (СтрНайти(НРег(ЗаданиеGitLab.name),"bdd server") > 0) Тогда
	...
	Для Каждого ЗаданиеGitLab Из ЗаданияGitLabСоСценарнымиТестами Цикл
		...
		Для Каждого ФайлjUnit Из ФайлыjUnit Цикл
			...
			Для Каждого testsuite Из testsuites.ДочерниеУзлы Цикл
				...
				Если ДанныеТеста.Свойство("StartDate") И ДанныеТеста.Свойство("EndDate") Тогда
					Попытка
	...
	Для Каждого ТекЗапуск Из ПодчиненныеЗапуски Цикл
		Длительность = Длительность + (ТекЗапуск.Длительность*3600);
	КонецЦикла;	
	ЗапускТестированияОбъект.Длительность = Окр(Длительность / 3600, 2);
	ЗаписатьЗапускТестирования(ЗапускТестированияОбъект);
...
Функция ИнформацияОЗапускеТестированияGitLab(Соединение, Токен, Проект, НомерЗапускТестированияGitLab)
	...
	Ресурс = СтрШаблон("/api/v4/projects/%1/pipelines/%2", СтрЗаменить(Проект, "/", "%2F"), Формат(НомерЗапускТестированияGitLab, "ЧГ="));
...
Функция НовыйЗапускТестирования(Параметры)
...
Функция АртефактыЗаданияGitLab(Соединение, Токен, Проект, НомерЗапускТестированияGitLab, ЗаданиеGitLab)
	Ресурс = СтрШаблон("/api/v4/projects/%1/jobs/%2/artifacts", СтрЗаменить(Проект, "/", "%2F"), Формат(ИдЗаданияGitlab(ЗаданиеGitLab), "ЧГ="));
...
Процедура СоздатьОшибкуЗапросаНаПомещение(Ветка)
	...
	Если ДанныеОткрытогоЗапросаНаСлияниеПоВетке.merge_status <> "cannot_be_merged" Тогда
		Возврат;
	КонецЕсли;	
...
Функция ВВеткеИсправленыВсеОшибки(Ветка)
...
Функция ДанныеПоЗаблокированнымТестамВВетке(Ветка, ДопПараметры)
	...
	ИначеЕсли ЕстьНеисправленныеОшибки Тогда                   
		...
		ДопПараметры.Вставить("ТребуетсяРешение", Ложь);  
	Иначе
		// Есть ошибки, которые блокирует выполнения тестов и исправляются в данной ветке, но они уже все исправлены или отозваны или есть упавшие тесты, но не зарегистрировавшие ошибки. 
