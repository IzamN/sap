...
Процедура ЗагрузитьИзмененияВВетках(Проект, Параметры = Неопределено) Экспорт
	...
	Для Каждого ДанныеВетки Из ДанныеВеток Цикл
		Попытка
			...
			Если ВВеткеЕстьИзмененияМетаданных(ДанныеВетки.Проект, ДанныеВетки.ПриемникИмя, ДанныеВетки.Имя, ДопПараметры) Тогда
	...
	ОповещенияДляРучныхИзменений(Проект);
...
Функция ДанныеВетокДляЗагрузкиИзмененийМетаданных(Проект, Параметры = Неопределено)
	...
	Иначе
		Запрос.Текст = 
			...
			|	Справочник.Ветки КАК Ветки
			|ГДЕ
			|	Ветки.Владелец.ЗагружатьИзмененияВВетках
...
Функция ДанныеСоответствияИмен()
    ...
    ДобавитьСоответсвиеИмен(Результат,"FunctionalOptionsParameters", "FunctionalOptionsParameter","ПараметрыФункциональныхОпций", "ПараметрФункциональныхОпций");
		...
		Для Каждого ДанныеВетки Из ДанныеВеткиПриемника Цикл
			...
			Для Каждого ТекМетаданное Из ИзмененныеМетаданные Цикл
				...
				МассивСтрокТаблицыИзменений = ТаблицаТекстИзменений.НайтиСтроки(
					Новый Структура("ОбъектМетаданных,ПодчиненныйОбъект", ЗаписьРС.ОбъектМетаданных, ЗаписьРС.ПодчиненныйОбъект));
				...
				ЗаписьРС.ТипИзменения = ТипИзмененияОбъектаМетаданныхGit(ЗаписьРС.ПодробноеОписаниеИзменений);
				
				Если ЗаписьРС.ТипИзменения = Перечисления.ТипыИзмененийОбъектовМетаданных.Добавлен
					И НЕ ЗначениеЗаполнено(ПредыдущееПодробноеОписаниеИзменений) Тогда		
				...
				ИначеЕсли ДанныеВетки.Ответственный = ДанныеМетаданногоСППР.Ответственный
				...
				Если НадоСделатьУведомлениеНовоеМетаданное Тогда
					...
					Если ЗначениеЗаполнено(ОтветственныйЗаПроект) Тогда
						ОтветственныеДляУведомленияНовыйОбъект.Вставить(ОтветственныйЗаПроект, Истина);
					...
					Иначе	
						...
						Если ДанныеТехПроектаПоВетке <> Неопределено И ЗначениеЗаполнено(ДанныеТехПроектаПоВетке.Ответственный) Тогда
							ОтветственныеДляУведомленияНовыйОбъект.Вставить(ДанныеТехПроектаПоВетке.Ответственный, Истина);
				...
				Если НадоСделатьУведомление Тогда
					Если ЗначениеЗаполнено(ДанныеМетаданногоСППР.СогласующийИзменения) Тогда
						ОтветственныеДляУведомленияИзменениеОбъекта.Вставить(ДанныеМетаданногоСППР.СогласующийИзменения, Истина);
					ИначеЕсли ЗначениеЗаполнено(ДанныеМетаданногоСППР.Ответственный) Тогда
				...
				Если ЗначениеЗаполнено(ПредыдущееПодробноеОписаниеИзменений) И
					...
					И ЗаписьРС.СтатусСогласования = Перечисления.СтатусыСогласованияИзмененийВВетках.ТребуетСогласования Тогда
			...
			ВыполнитьОповещениеОтветственныхПриИзмененииМетаданных(Проект, ОтветственныеДляУведомленияИзменениеОбъекта, ДанныеВетки, Ложь);
			ВыполнитьОповещениеОтветственныхПриИзмененииМетаданных(Проект, ОтветственныеДляУведомленияНовыйОбъект, ДанныеВетки, Истина);
...
Процедура ВыполнитьОповещениеОтветственныхПриИзмененииМетаданных(Проект, ОтветственныеДляУведомления, ДанныеВетки, ЭтоНовыйОбъект)
...
Процедура ОповещенияДляРучныхИзменений(Проект)
...
Функция ТипИзмененияОбъектаМетаданныхGit(ПодробноеОписаниеИзменений)
	
	НрегСтр = НРег(ПодробноеОписаниеИзменений);
