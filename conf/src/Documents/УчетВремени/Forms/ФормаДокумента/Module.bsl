...
Процедура ЗатратыВремениОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") Тогда
		...
		ОбработатьПодборРаботыНаСервере(НоваяСтрока.Работа, НоваяСтрока.ВидДеятельности, НоваяСтрока.ВидВремени,
		                                НоваяСтрока.ТребуемаяДетализация, НоваяСтрока.ТехническийПроект, НоваяСтрока.Этап, Истина);
...
Процедура ЗатратыВремениРаботаДляУчетаПриИзменении(Элемент)
	...
	СтрокаЗатрат.ВидДеятельности = ПредопределенноеЗначение("Справочник.ВидыДеятельности.ПустаяСсылка");
...
Процедура ЗатратыВремениОписаниеРаботыОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	ИзменятьПоляПриВыбореИзАвтоподбора = ИзменятьПоляПриВыбореИзАвтоподбора(); 
	
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") Тогда
		
		СтрокаТЧ = Элементы.ЗатратыВремени.ТекущиеДанные;
		Если НЕ ИзменятьПоляПриВыбореИзАвтоподбора Тогда
		
			СтрокаТЧ.ВидДеятельности = ПредопределенноеЗначение("Справочник.ВидыДеятельности.ПустаяСсылка");	
...
&НаСервереБезКонтекста
Процедура ОбработатьПодборРаботыНаСервере(Работа, ВидДеятельности, ВидВремени, ТребуемаяДетализация, ТехническийПроект, Этап, ЗаполнятьВидВремени)
...
Процедура ПриИзмененииРаботыНаСервере(НомерСтроки)
	...
	Если ТипЗнч(СтрокаЗатрат.Работа) = Тип("СправочникСсылка.ЗадачиПроцесса") Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(РеквизитыЗадачи.Проект) Тогда
			...
			СтрокаЗатрат.Этап = РеквизитыЗадачи.Этап;
		КонецЕсли;
	...
	ИначеЕсли ТипЗнч(СтрокаЗатрат.Работа) = Тип("СправочникСсылка.Ошибки") Тогда
		...
		Если ИспользоватьПроектПоУмолчаниюДляВсехРабот И ЗначениеЗаполнено(ПроектПоУмолчанию) Тогда
			СтрокаЗатрат.Проект = ПроектПоУмолчанию;
		ИначеЕсли ЗначениеЗаполнено(РеквизитыОшибки.Проект) И ЗначениеЗаполнено(РеквизитыОшибки.Этап) Тогда
			СтрокаЗатрат.Проект = РеквизитыОшибки.Проект;
			СтрокаЗатрат.Этап = РеквизитыОшибки.Этап;
		КонецЕсли;
	...
	//04.08.2023 ЭДИТПРО ЗиганшинД Автоматическое заполнение проекта, исходя из указанного в тех.проекте
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ТехническиеПроекты.Владелец.Ссылка КАК Проект,
	|	ТехническиеПроекты.Владелец.ВидДеятельности КАК ВидДеятельности,
	...
	Запрос.УстановитьПараметр("Ссылка", СтрокаЗатрат.ТехническийПроект);
	
	РезультатЗапроса = Запрос.Выполнить();
	Если НЕ РезультатЗапроса.Пустой() Тогда
		Выгрузка = РезультатЗапроса.Выгрузить();
		Проект = Выгрузка.Получить(0).Проект;
		ВидДеятельности = Выгрузка.Получить(0).ВидДеятельности;
		Этап = Выгрузка.Получить(0).РазделПроекта;
		Если НЕ ЗначениеЗаполнено(СтрокаЗатрат.ВидДеятельности) Тогда
			СтрокаЗатрат.ВидДеятельности = ВидДеятельности;
		КонецЕсли;
		СтрокаЗатрат.Проект = Проект;
		СтрокаЗатрат.Этап = Этап;
	КонецЕсли;
	//04.08.2023
...
Функция РеквизитыЗадачи(Задача)
	...
	Результат.Вставить("Этап", Справочники.РазделыПроекта.ПустаяСсылка());
	...
	|		КОГДА ЗадачиПроцесса.Предмет ССЫЛКА Справочник.СборкиВерсии
	|				ИЛИ ЗадачиПроцесса.Предмет ССЫЛКА Справочник.ФункцииМеханизмов
	|			ТОГДА ЗадачиПроцесса.Предмет.Владелец.Владелец.ВидДеятельности
	|		ИНАЧЕ ЗадачиПроцесса.Предмет.Владелец.ВидДеятельности
	...
	Если Выборка.Следующий() Тогда
		...
		//Глинский А.А. ЭП-0026353 06.03.2025 {  
		//Результат.Вставить("Этап", Выборка.ТехническийПроект);
		Результат.Вставить("Этап", Выборка.Этап); 
		//Глинский А.А. ЭП-0026353 06.03.2025 }		
...
Процедура ОбработкаВыбораРаботы(РезультатВыбора, Строка)
	...
	ОбработатьПодборРаботыНаСервере(Строка.Работа, Строка.ВидДеятельности, Строка.ВидВремени, Строка.ТребуемаяДетализация,
	                                Строка.ТехническийПроект, Строка.Этап, ЗаполнятьВидВремени);
...
Функция ДобавитьОшибкуПоКодуНаСервере(Код)
	...
	Если ЗначениеЗаполнено(РеквизитыОшибки.Ссылка) Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(РеквизитыОшибки.Проект) Тогда
			...
			НоваяСтрока.Этап = РеквизитыОшибки.Этап;
		КонецЕсли;
...
Функция ДобавитьТехническийПроектПоКодуНаСервере(Код)
	...
	|	ТехническиеПроекты.РазделПроекта КАК Этап,
	|	ТехническиеПроекты.Владелец.ВидДеятельности КАК ВладелецВидДеятельности,
	|	ТехническиеПроекты.Владелец.ВидДеятельности.ВидВремени КАК ВидВремени
	|ИЗ
	|	Справочник.ТехническиеПроекты КАК ТехническиеПроекты
	|ГДЕ
	...
	Если Выборка.Следующий() Тогда
		...
		Если ИспользоватьПроектПоУмолчаниюДляВсехРабот И ЗначениеЗаполнено(ПроектПоУмолчанию) Тогда
			НоваяСтрока.Проект = ПроектПоУмолчанию;
		Иначе
			НоваяСтрока.ВидДеятельности = Выборка.ВладелецВидДеятельности;
...
&НаСервере
Функция ИзменятьПоляПриВыбореИзАвтоподбора()

	Возврат ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
		"УчетВремени", 
		"ОтключитьАвтоматическоеЗаполнениеПолейПриВыбореОписанияРаботыИзСписка", 
		Ложь);	
