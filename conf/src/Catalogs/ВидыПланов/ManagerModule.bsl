...
Процедура ЗарегистрироватьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ИЗ
	|	Справочник.ВидыПланов КАК ВидыПланов
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыПланов.ВладельцыПлана КАК ВидыПлановВладельцыПлана
	|		ПО (ВидыПлановВладельцыПлана.Ссылка = ВидыПланов.Ссылка)
	|			И (ВидыПлановВладельцыПлана.Владелец = ВидыПланов.УдалитьВладелецПлана)
	|ГДЕ
	|	(ВидыПлановВладельцыПлана.Ссылка ЕСТЬ NULL
	|	И ВидыПланов.УдалитьВладелецПлана <> ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка))
	|	ИЛИ ВидыПланов.ТипПлана = ЗНАЧЕНИЕ(Перечисление.ТипыПланов.ПустаяСсылка)";
...
Процедура ОбработатьДанныеДляПереносаВладельцевВТабличнуюЧасть(Параметры) Экспорт
	...
	|ВЫБРАТЬ
	|	ВидыПланов.Ссылка       КАК Ссылка,
	|	ВидыПланов.Наименование КАК НаименованиеВидаПлана,
	|	ВЫБОР
	|		КОГДА (ВидыПланов.УдалитьВладелецПлана <> ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка)
	|				И ВидыПлановВладельцыПлана.Ссылка ЕСТЬ NULL)
	|				ИЛИ ВидыПланов.ТипПлана = ЗНАЧЕНИЕ(Перечисление.ТипыПланов.ПустаяСсылка) 
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	...
	|			И (ВидыПлановВладельцыПлана.Владелец = ВидыПланов.УдалитьВладелецПлана)
	|ГДЕ
	|	ВидыПланов.Ссылка В(&ЭлементыДляОбработки)";
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если Не ЗначениеЗаполнено(СправочникОбъект.УдалитьВладелецПлана) Тогда
				ОтменитьТранзакцию();
				ОбновлениеИнформационнойБазы.ОтметитьВыполнениеОбработки(Выборка.Ссылка);
				Продолжить;
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(СправочникОбъект.ТипПлана) Тогда
				СправочникОбъект.ТипПлана = Перечисления.ТипыПланов.Рабочий;
			КонецЕсли;
			
			Если ЗначениеЗаполнено(СправочникОбъект.УдалитьВладелецПлана) Тогда
				
				ПараметрыПоиска = Новый Структура;
				ПараметрыПоиска.Вставить("Владелец", СправочникОбъект.УдалитьВладелецПлана);
				
				Если СправочникОбъект.ВладельцыПлана.НайтиСтроки(ПараметрыПоиска).Количество() = 0 Тогда
					НоваяСтрока = СправочникОбъект.ВладельцыПлана.Добавить(); 
					НоваяСтрока.Владелец                  = СправочникОбъект.УдалитьВладелецПлана;
				КонецЕсли;
				
				СправочникОбъект.УдалитьВладелецПлана = Справочники.Пользователи.ПустаяСсылка();
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Не удалось перенести владельца вида плана в табличную часть для объекта %1 по причине: %2'"),
			                           Выборка.НаименованиеВидаПлана,
