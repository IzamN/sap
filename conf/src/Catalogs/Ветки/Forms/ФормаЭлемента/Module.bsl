...
Процедура ПроверитьИсправлениеОшибокВВетке(Команда)
	...
	РезультатПроверки = ПроверкаИсправленияОшибокВВетке();
	Если НЕ РезультатПроверки.ПроверкаВыполненаУспешно Тогда
		Если РезультатПроверки.ОшибкиУКоторыхНетКоммитов.Количество() > 0 
			...
			РезультатПроверки.Вставить("Ветка", Объект.Ссылка);
			ОткрытьФорму("Справочник.Ветки.Форма.ПроверкаИсправленияОшибокВВетке", РезультатПроверки, ЭтаФорма);
		Иначе
...
&НаСервере
Функция ПроверкаИсправленияОшибокВВетке()
	
	Возврат Справочники.Ветки.ПроверкаИсправленияОшибокВВетке(Объект.Ссылка, Истина);
...
Функция ГиперссылкаНаЗапросНаСлияние(ИдентификаторЗапросаНаСлияние = Неопределено)
	...
	ДанныеПодключенияПроектаКGitСерверу = Тестирование.ДанныеПодключенияПроектаКGitСерверу(Объект.Владелец);
