...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("СсылкаНаОшибку") И ЗначениеЗаполнено(Параметры.СсылкаНаОшибку) Тогда
		...
		Если МассивСтрок.Количество() > 0 Тогда
			...
			ТекстСообщения = ТекстИсправления;
		КонецЕсли;	 
	КонецЕсли;	 
...
Процедура ПриОткрытии(Отказ)
	...
	ИмяВеткиМерджРеквеста = "bdd/" + ИмяВеткиВGit(Ветка) + "-"
		+ СтрЗаменить(СтроковыеФункцииКлиентСервер.СтрокаЛатиницей(ПользователиКлиент.ТекущийПользователь()), " ", "");
		
	Если ДанныеРанееИзмененныхСценариев.ИзмененныеUID.Количество() > 0 Тогда
		ИмяВеткиМерджРеквеста = ИмяВеткиМерджРеквеста + "-" + ДанныеРанееИзмененныхСценариев.ИзмененныеUID[0].Код;
	КонецЕсли;	 	
	
	ИмяВеткиМерджРеквеста = ИмяВеткиМерджРеквеста + "-" + Формат(ТекущаяДата(), "ДФ=yyyyMMddччммсс");
