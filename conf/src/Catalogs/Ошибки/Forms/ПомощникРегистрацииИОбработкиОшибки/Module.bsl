...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИнтеграцияС1СДокументооборот.ПриСозданииНаСервере(ЭтаФорма, Элементы.ГруппаКоманднаяПанельФормы);
...
Процедура СоздатьВеткиДляИсправления(Команда)
	...
	Для Каждого Строка из Объект.ИсправлениеВВетках Цикл
		Если (Строка.СостояниеИСправления = ПредопределенноеЗначение("Перечисление.СостоянияИсправленияОшибок.ТребуетсяИсправление")
			...
			Строка.ВеткаИсправления = ВеткаИсправленияПоОшибке(Строка.Ветка);
		КонецЕсли;
	КонецЦикла;
...
Процедура УстановитьВидимостьЭлементовАудитаКода()
	...
	Если МетодВыявления = Перечисления.МетодыВыявленияОшибок.АудитКода Тогда
		...
		Элементы.ГруппаИБ.Видимость = Ложь;
		Элементы.ГруппаСтандарт.Видимость = Истина;
		...
		Элементы.ВидПроблемы.Видимость = Ложь;
		
	ИначеЕсли МетодВыявления = Перечисления.МетодыВыявленияОшибок.РаботаСПрограммой Тогда
		
		Элементы.ГруппаИБ.Видимость = Истина;
		Элементы.ГруппаСтандарт.Видимость = Ложь;
		Если Не ЗначениеЗаполнено(Объект.Наименование) Тогда
			Элементы.Наименование.ОтметкаНезаполненного = Истина;
		КонецЕсли;
	...
	Иначе
		...
		Элементы.ВидПроблемы.Видимость = Истина;
...
Процедура УстановитьВерсиюИсправления()
	...
	Если НЕ Объект.Исправлена Тогда
		...
		Если Объект.Обнаружена = Перечисления.МестаОбнаруженияОшибок.ВВетке
			И Объект.Исправляется = Перечисления.ВариантыИсправленияОшибок.ТолькоВВеткеОбнаружения
			И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ВеткаОбнаружения, "Тип") <> Перечисления.ТипыВеток.ОсновнаяВеткаПроекта Тогда
			
			// Если ошибка обнаружена в ветке исправляется там же,
			// версия исправления совпадает с версией обнаружения.
			Объект.ВерсияИсправления = Справочники.Ошибки.ВерсияОбнаруженияДляВетки(Объект.ВеткаОбнаружения);
		Иначе	
			Объект.ВерсияИсправления = 
			    Справочники.Ошибки.ВерсияИсправления(Объект.Владелец,
			                                         Объект.Обнаружена,
...
&НаСервере
Процедура УстановитьСписокВыбораКомуНаправлена()
	...
	Если ЗначениеЗаполнено(Объект.Зарегистрировал) И Объект.Зарегистрировал <> Объект.Автор Тогда
		СписокЗначений.Добавить(Объект.Зарегистрировал, "Зарегистрировал: " + Объект.Зарегистрировал);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Объект.Рассмотрел) Тогда
		СписокЗначений.Добавить(Объект.Рассмотрел, "Рассмотрел: " + Объект.Рассмотрел);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Объект.Исправил) Тогда
		СписокЗначений.Добавить(Объект.Исправил, "Исправил: " + Объект.Исправил);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Объект.Проверил) Тогда
	...
	Если УказыватьРазделПроекта И ЗначениеЗаполнено(ОтветственныйЗаРаздел) Тогда
...
Процедура АктуализироватьИсправлениеВВетках()
	...
	ВеткиДляИсправления = Справочники.Ошибки.ВеткиДляИсправленияОшибки(Объект.Владелец, Объект.ВерсияИсправления);
	...
	Для Каждого СтрокаТаблицы из ВеткиДляИсправления ЦИкл
		...
		Если Объект.ИсправлениеВВетках.НайтиСтроки(СтруктураОтбора).Количество()=0 Тогда
...
Процедура ЗаполнитьСписокТехническихПроектовОбнаружения()
	...
	Если ТипВеткиОбнаружения = Перечисления.ТипыВеток.ВеткаТехническогоПроекта Тогда
...
&НаСервере
Функция ВеткаИсправленияПоОшибке(Источник)
	
	Возврат Справочники.Ветки.НоваяВетка(Объект, Новый Структура("Источник", Источник));
