...
Процедура ЗарегистрироватьДанныеКОбработкеЗаполненияВеткиИсточника(Параметры) Экспорт
	...
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ПредопределенныеДанные.Ссылка КАК Ссылка
	|ИЗ
	|	Справочник.ПредопределенныеДанные КАК ПредопределенныеДанные
	|ГДЕ
	|	ПредопределенныеДанные.УдалитьТехническийПроектИсточник <> ЗНАЧЕНИЕ(Справочник.ТехническиеПроекты.ПустаяСсылка)
	|	И ПредопределенныеДанные.УдалитьТехническийПроектИсточник.Ветка.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыВеток.ВеткаТехническогоПроекта)
	|	И ПредопределенныеДанные.ВеткаИсточник = ЗНАЧЕНИЕ(Справочник.Ветки.ПустаяСсылка)";
...
Процедура ОбработатьДанныеДляЗаполненияВеткиИсточника(Параметры) Экспорт
	...
	ПолноеИмяОбъекта = "Справочник.ПредопределенныеДанные";
	...
	"ВЫБРАТЬ
	|	ПредопределенныеДанные.Ссылка КАК Ссылка,
	|	ПредопределенныеДанные.УдалитьТехническийПроектИсточник.Ветка КАК ВеткаИсточник
	|ИЗ
	|	Справочник.ПредопределенныеДанные КАК ПредопределенныеДанные
	|ГДЕ
	|	ПредопределенныеДанные.Ссылка В(&СписокОбъектов)"
	;
