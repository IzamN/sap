...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Эдит-Про 20221213 СкрипникДА
	НастроитьСписокСотрудников();
	// _Эдит-Про 20221213 СкрипникДА
	НастроитьСписокСотрудниковПлановаяЗагрузка();
	НастроитьСписокВиртуальныхЭтапов();
	
	ТекущийПользователь = Пользователи.ТекущийПользователь();
	
	Если Пользователи.РолиДоступны("ЭП_УтверждениеТрудозатратКПремированию", ТекущийПользователь) 
		ИЛИ Пользователи.РолиДоступны("ЭП_ПодготовкаДокументовТрудозатратыКПремированию", ТекущийПользователь)
		ИЛИ Пользователи.РолиДоступны("ЭП_СогласованиеТрудозатратКПремированию", ТекущийПользователь) Тогда
		Элементы.УведомлениеРуководителей.Доступность = Истина;
	Иначе
			Элементы.УведомлениеРуководителей.Доступность = Ложь;
	КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ОбработкаЭП_АРМПомощникСозданияПроекта" Тогда
...
Процедура УстановитьДоступностьЭлементов()
	...
	Элементы.ГруппаРазработкаВEDT.Видимость = (РазработкаВGitРепозитории);
	...
	Элементы.АдресСервераСценарииРаботыПользователя.Видимость =
		Объект.ХранитьСценарииРаботыПользователейВРепозитории ИЛИ Объект.ЗагружатьРезультатыВыполненияТестов;
	Элементы.РедактированиеСценариевВGitИмяПроекта.Видимость =
		Объект.ХранитьСценарииРаботыПользователейВРепозитории ИЛИ Объект.ЗагружатьРезультатыВыполненияТестов;
...
// Эдит-Про 20221213 СкрипникДА
#Область ДоработкиЭдитПро

&НаСервере
Процедура НастроитьСписокСотрудников()
	
	ТекВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(СписокСотрудников, "Проект", Объект.Ссылка, ТекВидСравнения,,Истина);
	
КонецПроцедуры  

&НаСервере
Процедура НастроитьСписокВиртуальныхЭтапов()

	ТекВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(СписокВиртуальныхЭтапов, "Проект", Объект.Ссылка, ТекВидСравнения,,Истина);	
...
Процедура СписокСотрудниковПередУдалением(Элемент, Отказ)
	...
	ПараметрыУдаления.Вставить("Сотрудник", ТекущиеДанные.Сотрудник);
	ПараметрыУдаления.Вставить("Проект", Объект.Ссылка);
...
&НаКлиенте
Процедура ЭП_РасчетноеКоличествоСотрудниковПриИзменении(Элемент)
	ПодсчитатьСотрудниковПроекта();
КонецПроцедуры

&НаКлиенте
Процедура ЭП_ОбновитьКоличествоСотрудников(Команда)
	ПодсчитатьСотрудниковПроекта();
КонецПроцедуры

&НаСервере
Процедура ПодсчитатьСотрудниковПроекта()
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЭП_ЗагруженностьСотрудниковСрезПоследних.Сотрудник) КАК КоличествоСотрудников
	|ИЗ
	...
	ЭП_УчастникиПроекта = Выборка.КоличествоСотрудников; 
	ЭП_ПотребностьВЛюдях = Объект.ЭП_РасчетноеКоличествоСотрудников - ЭП_УчастникиПроекта;
...
Процедура ЗагруженностьОткрытьФормуЗаписи(РежимДобавления)
	...
	Иначе
		...
		ПараметрыЗаполнения = Новый Структура("Сотрудник,ПроцентЗагрузки,СрокНахожденияВПроекте");
		ЗаполнитьЗначенияСвойств(ПараметрыЗаполнения,ТекущиеДанные);
	КонецЕсли;
	
	ПараметрыЗаполнения.Вставить("Период", ТекущаяДата());
	ПараметрыЗаполнения.Вставить("Проект", Объект.Ссылка);
	
	ПараметрыОткрытия = Новый Структура("РежимФормы, ЗначенияЗаполнения", "Проект", ПараметрыЗаполнения);
		
	ОткрытьФорму("РегистрСведений.ЭП_ЗагруженностьСотрудников.ФормаЗаписи", ПараметрыОткрытия, Элементы.СписокСотрудников,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	
КонецПроцедуры
  
&НаКлиенте
Процедура ВиртуальныеЭтапыОткрытьФормуЗаписи()

	ПараметрыЗаполнения = Новый Структура; 
	ПараметрыЗаполнения.Вставить("КодБИ", Объект.ЭП_КодBI);
	ПараметрыЗаполнения.Вставить("Проект", Объект.Ссылка);
			
	ПараметрыОткрытия = Новый Структура("ЗначенияЗаполнения", ПараметрыЗаполнения);
		
	ОткрытьФорму("РегистрСведений.ЭП_ВиртуальныеЭтапыПроектов.ФормаЗаписи", ПараметрыОткрытия, Элементы.СписокВиртуальныхЭтапов,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);	
...
Процедура ГруппаСтраницыПриСменеСтраницыНаСервере()
	...
	Элементы.ЭП_ТаблицаОцениванияОценивает.СписокВыбора.Очистить();
	Выборка = РезультатЗапроса.Выбрать();
	Пока Выборка.Следующий() Цикл
	
		Элементы.ЭП_ТаблицаОцениванияОценивает.СписокВыбора.Добавить(Выборка.Сотрудник);	
...
Процедура ГруппаСтраницыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Если ТекущаяСтраница.Имя = "ЭП_НастройкаОценивания" Тогда
...
&НаСервере
Процедура НастроитьСписокСотрудниковПлановаяЗагрузка()
	
	ТекВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(СписокСотрудниковПлан, "Проект", Объект.Ссылка, ТекВидСравнения,,Истина);
	
КонецПроцедуры

&НаКлиенте
Процедура СписокСотрудниковПланПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	Отказ = Истина;
	ЗагруженностьОткрытьФормуЗаписиПлан(Истина);
КонецПроцедуры 

&НаКлиенте
Процедура СписокСотрудниковПланПередНачаломИзменения(Элемент, Отказ)
	Отказ = Истина;
	ЗагруженностьОткрытьФормуЗаписиПлан(Ложь);
КонецПроцедуры

&НаКлиенте
Процедура ЗагруженностьОткрытьФормуЗаписиПлан(РежимДобавления)

	Если РежимДобавления Тогда
		ПараметрыЗаполнения = Новый Структура;
	Иначе
		ТекущиеДанные = Элементы.СписокСотрудниковПлан.ТекущиеДанные;
		Если ТекущиеДанные = Неопределено Тогда
			Возврат;       
		КонецЕсли;

		ПараметрыЗаполнения = Новый Структура(
							"Сотрудник,ПлановыйПроцентЗагрузки,СрокНахожденияВПроекте,Этап,РольСотрудника,ФункциональноеНаправление,ТипРесурса,Департамент");
		ЗаполнитьЗначенияСвойств(ПараметрыЗаполнения,ТекущиеДанные);
	КонецЕсли;
	...
	ОткрытьФорму("РегистрСведений.ЭП_ПлановаяЗагруженностьСотрудников.ФормаЗаписи", ПараметрыОткрытия, Элементы.СписокСотрудниковПлан,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	
КонецПроцедуры

&НаКлиенте
Процедура СписокСотрудниковПланПередУдалением(Элемент, Отказ)
	Отказ = Истина;
	...
	ПараметрыУдаления.Вставить("Сотрудник", ТекущиеДанные.Сотрудник);
	ПараметрыУдаления.Вставить("Проект", Объект.Ссылка); 
	ПараметрыУдаления.Вставить("Этап", ТекущиеДанные.Этап);
	ПараметрыУдаления.Вставить("РольСотрудника", ТекущиеДанные.РольСотрудника);
	ПараметрыУдаления.Вставить("ФункциональноеНаправление", ТекущиеДанные.ФункциональноеНаправление);

	ЗагруженностьПланУдалитьНаСервере(ПараметрыУдаления);
	Элементы.СписокСотрудниковПлан.Обновить();

КонецПроцедуры 

&НаСервере
Процедура ЗагруженностьПланУдалитьНаСервере(ПараметрыУдаления)
...
&НаКлиенте
Процедура СписокВиртуальныхЭтаповПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	Отказ = Истина;
	ВиртуальныеЭтапыОткрытьФормуЗаписи();
КонецПроцедуры

&НаКлиенте
Процедура СписокВиртуальныхЭтаповПередНачаломИзменения(Элемент, Отказ)
	//Отказ = Истина;
	//ВиртуальныеЭтапыОткрытьФормуЗаписи();
КонецПроцедуры

&НаКлиенте
Процедура СоздатьВиртуальныйЭтап(Команда)
...
Процедура ОбновитьСпискоВиртуальныхЭтапов(Результат, ДопПараметры) экспорт
	   Элементы.СписокВиртуальныхЭтапов.Обновить();
КонецПроцедуры

&НаКлиенте
Процедура ЭП_АрхивныйПриИзменении(Элемент)
	
	ЭП_АрхивныйПриИзмененииНаСервере();

КонецПроцедуры

&НаСервере
Процедура ЭП_АрхивныйПриИзмененииНаСервере()

	Если Объект.ЭП_Архивный = Перечисления.ЭП_СтатусПроекта.Архивный ИЛИ
						Объект.ЭП_Архивный = Перечисления.ЭП_СтатусПроекта.АрхивныйСПросмотром Тогда
						
		Объект.ЭП_ДатаОтправкиВАрхив = ТекущаяДатаСеанса();  
		
	Иначе               
		
		Объект.ЭП_ДатаОтправкиВАрхив = Дата(1,1,1);	 
...
&НаКлиенте
Процедура ЭП_НастройкаПроекта(Команда)
	
	ПараметрыФормы = Новый Структура("НастройкаПроекта", Объект.Ссылка);
	ОткрытьФорму("Обработка.ЭП_АРМПомощникСозданияПроекта.Форма.Форма",ПараметрыФормы,ЭтаФорма);
