...
Процедура ЗарегистрироватьДанныеКОбработкеЗаполненияВеткиИсточника(Параметры) Экспорт
	...
	|ГДЕ
	|	КомандыОбъектовМетаданных.УдалитьТехническийПроектИсточник <> ЗНАЧЕНИЕ(Справочник.ТехническиеПроекты.ПустаяСсылка)
	|	И КомандыОбъектовМетаданных.УдалитьТехническийПроектИсточник.Ветка.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыВеток.ВеткаТехническогоПроекта)
	|	И КомандыОбъектовМетаданных.ВеткаИсточник = ЗНАЧЕНИЕ(Справочник.Ветки.ПустаяСсылка)";
...
Процедура ОбработатьДанныеДляЗаполненияВеткиИсточника(Параметры) Экспорт
	...
	ПолноеИмяОбъекта = "Справочник.КомандыОбъектовМетаданных";
	...
	"ВЫБРАТЬ
	|	КомандыОбъектовМетаданных.Ссылка КАК Ссылка,
	|	КомандыОбъектовМетаданных.УдалитьТехническийПроектИсточник.Ветка КАК ВеткаИсточник
	|ИЗ
	|	Справочник.КомандыОбъектовМетаданных КАК КомандыОбъектовМетаданных
	|ГДЕ
	|	КомандыОбъектовМетаданных.Ссылка В(&СписокОбъектов)"
	;
