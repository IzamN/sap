...
Процедура ПередЗаписью(Отказ)
	...
	Если НЕ ЭтоГруппа Тогда
		...
		Иначе
			ЭП_ЗафиксированнаяДатаИзменения = ТекущаяДатаСеанса();
	...
	Если НЕ ЭтоГруппа Тогда
		...
		СтарыеЗначения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Ссылка, "ЦелиПроекта, КонцепцияПроекта");
		
		Если НЕ СтарыеЗначения.ЦелиПроекта = ЦелиПроекта ИЛИ НЕ СтарыеЗначения.КонцепцияПроекта = КонцепцияПроекта Тогда
			
			НоваяДата = НачалоДня(ТекущаяДатаСеанса());
			Если НЕ ЭП_ЗафиксированнаяДатаИзменения = НоваяДата Тогда
				
				ЭП_ЗафиксированнаяДатаИзменения = НоваяДата;
...
Процедура ПриЗаписи(Отказ)
	...
	Если ОбменДанными.Загрузка Тогда
		Возврат;
	КонецЕсли; 
	Если Статус = Перечисления.СтатусыТехническихПроектов.Активен И ПустойПодстатус() Тогда
	
		УстановитьПривилегированныйРежим(Истина);
		
		МенеджерПодстатуса = РегистрыСведений.ЭП_ПодстатусыТехническихПроектов.СоздатьМенеджерЗаписи();
		МенеджерПодстатуса.Период = ТекущаяДатаСеанса();
		МенеджерПодстатуса.Подстатус = Перечисления.ЭП_Подстатусы.НаписаниеПостановки;
		МенеджерПодстатуса.Комментарий = "Автоматическое заполнение пустого подстатуса в активном техническом проекте";
		МенеджерПодстатуса.ТехПроект = Ссылка;
		МенеджерПодстатуса.Пользователь = ПараметрыСеанса.ТекущийПользователь;
		МенеджерПодстатуса.Записать();
...
Процедура ПроверитьВозможностьЗаписиСИзмененнымиРеквизитами(Отказ);
	...
	СтрокаРеквизитов = "ПлановаяДатаНачала,ПлановаяДатаОкончания,Очередность,Версия,Публикуется";
	Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Ссылка, СтрокаРеквизитов);
		
	Если НЕ ДоступноУправлениеТехническимПроектом
		И Статус<>Перечисления.СтатусыТехническихПроектов.НеЗапланирован
		И Статус<>Перечисления.СтатусыТехническихПроектов.Запланирован Тогда
		
		Если Реквизиты.ПлановаяДатаНачала <> ПлановаяДатаНачала Тогда
			Отказ = Истина;
			ТекстСообщения = 
			НСтр("ru='Текущему пользователю запрещено изменять значение реквизита ""Плановая дата начала"" в техническом проекте %ТехническийПроект%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТехническийПроект%", Ссылка);
			ВызватьИсключение(ТекстСообщения);
		КонецЕсли;
		
		Если Реквизиты.ПлановаяДатаОкончания <> ПлановаяДатаОкончания Тогда
			Отказ = Истина;
			ТекстСообщения = 
			НСтр("ru='Текущему пользователю запрещено изменять значение реквизита ""Плановая дата окончания"" в техническом проекте %ТехническийПроект%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТехническийПроект%", Ссылка);
			ВызватьИсключение(ТекстСообщения);
		КонецЕсли;
...
Функция ПустойПодстатус()
	...
	Запрос.УстановитьПараметр("ТехПроект", Ссылка);
