...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// СкрипникДА 20230104  
	ЭП_ПриСозданииНаСервере(Отказ, СтандартнаяОбработка);
	Если Объект.Ссылка.Пустая() Тогда	
		ЭП_ЗаполнитьВерсию();
		Объект.Ответственный = Пользователи.ТекущийПользователь();
	КонецЕсли;
	// _СкрипникДА 20230104 
	

	//Заполнение надписи тегов
	
	Если ЗначениеЗаполнено(Объект.ЭП_РеализацияУслуг) Тогда 
		Элементы.ДекорацияДокументРеализации.Заголовок = Строка(Объект.ЭП_РеализацияУслуг);
		Элементы.ДекорацияДокументРеализации.Гиперссылка = Истина;
		Если РольДоступна("Администрирование") Или РольДоступна("ПолныеПрава") 
			Или РольДоступна("АдминистраторСистемы") Тогда 
			Элементы.КнопкаИзменитьРеализацию.Доступность = Истина;
		Иначе
			Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
			                      ...
			                      |ГДЕ
			                      |	ЭП_ВыплатыПремийСотрудники.ТехническийПроект = &ТехническийПроект
			                      |	И ЭП_ВыплатыПремий.Проведен");
			Запрос.УстановитьПараметр("ТехническийПроект", Объект.Ссылка);
			Результат = Запрос.Выполнить();
			Элементы.КнопкаИзменитьРеализацию.Доступность = Результат.Пустой();
		КонецЕсли;
	Иначе 
		Элементы.ДекорацияДокументРеализации.Гиперссылка = Ложь;
		Элементы.ДекорацияДокументРеализации.Заголовок = "Документ реализации не выбран";
	КонецЕсли; 
	
	Если ЗначениеЗаполнено(Объект.Владелец) И Объект.Владелец.ЭП_ПроектСопровождение Тогда 
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ИдеяИзменена" Тогда
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ИсточникВыбора.ИмяФормы = "Документ.ЭП_РеализацияУслуг.ФормаВыбора" 
			И ЗначениеЗаполнено(ВыбранноеЗначение) Тогда
		Объект.ЭП_РеализацияУслуг = ВыбранноеЗначение;	 
		Элементы.ДекорацияДокументРеализации.Заголовок = Строка(Объект.ЭП_РеализацияУслуг);
		Элементы.ДекорацияДокументРеализации.Гиперссылка = Истина;
		ЭтаФорма.Модифицированность = Истина;   
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗавершениеОтветаНаВопросИзменениеСтатуса", ЭтотОбъект), 
					   "Перевести технический проект в архив?", 
...
&НаКлиенте
Процедура ЗавершениеОтветаНаВопросИзменениеСтатуса(Результат, Параметры) Экспорт
	...
	Объект.ЭП_ДатаПомещенияВАрхив = ТекущаяДата();
	Объект.ЭП_Архивный = Истина;
...
Процедура ГруппаСтраницыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Если ТекущаяСтраница = Элементы.Зависимости Тогда
		Если ЗначениеЗаполнено(Объект.ЭП_Зависимости) Тогда	
...
Процедура ПриИзмененииВладельцаСервер()
	...
	Если Объект.Владелец.ЭП_ПроектСопровождение Тогда 
		Элементы.ГруппаРеализация.Видимость = Истина;
	Иначе
		Элементы.ГруппаРеализация.Видимость = Ложь;
		Объект.ЭП_РеализацияУслуг = Документы.ЭП_РеализацияУслуг.ПустаяСсылка();
        Элементы.ДекорацияДокументРеализации.Гиперссылка = Ложь;
		Элементы.ДекорацияДокументРеализации.Заголовок = "Документ реализации не выбран"; 
	КонецЕсли;   
...
Процедура УстановитьДоступностьЭлементовПланирования(Элементы, Статус, ДоступноУправлениеТехническимПроектом)
	...
	Элементы.ПлановаяДатаНачала.ТолькоПросмотр = Не ДоступноУправлениеДатами;
	Элементы.ПлановаяДатаОкончания.ТолькоПросмотр = Не ДоступноУправлениеДатами;
...
#Область ЭП_Доработки

&НаСервере
Процедура ЭП_ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	
	// Добавить команду
	НоваяКоманда = Команды.Добавить("ЭП_ДобавитьИдею");
	НоваяКоманда.Действие = "ЭП_ДобавитьИдею";
	НоваяКоманда.Заголовок = "Добавить идею";
	
	// Добавить кнопку на панель списка "Идеи"
	ГруппаЭлементов = Элементы.Найти("ИдеиКоманднаяПанель");
	
	НовыйЭлемент = Элементы.Вставить("ИдеиЭП_ДобавитьИдею", Тип("КнопкаФормы"), ГруппаЭлементов, ГруппаЭлементов.ПодчиненныеЭлементы[0]);
	НовыйЭлемент.ИмяКоманды = "ЭП_ДобавитьИдею";
	
	// Добавить обработчик события
	Элементы.Идеи.УстановитьДействие("ОбработкаЗаписиНового", "ЭП_ИдеиОбработкаЗаписиНового");
	
	// Добавить колонки ТЧ "Идеи"
	ЭлементРодитель = Элементы.Найти("Идеи");
	
	НовыйЭлемент = Элементы.Добавить("ИдеиЭП_ТрудозатратыАналитика", Тип("ПолеФормы"), ЭлементРодитель);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.ПутьКДанным = "Объект.ИдеиИОшибки.ЭП_ТрудозатратыАналитика";

	НовыйЭлемент = Элементы.Добавить("ИдеиЭП_ТрудозатратыРазработчика", Тип("ПолеФормы"), ЭлементРодитель);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.ПутьКДанным = "Объект.ИдеиИОшибки.ЭП_ТрудозатратыРазработчика";
	
	// Переместить вкладку "Идеи" во вторую позицию
	ГруппаСтраницы = Элементы.Найти("ГруппаСтраницы");
	СтраницаИдеи = Элементы.Найти("ВходящиеИдеиИОшибки");
	СтраницаУчастники = Элементы.Найти("СтраницаУчастники");
	Элементы.Переместить(СтраницаИдеи, ГруппаСтраницы, СтраницаУчастники);
	
	// Отключить гиперссылку в ячейке "ИдеяОшибка"
	ЭлементФормы = Элементы.Найти("ИдеиИдея");
	ЭлементФормы.ГиперссылкаЯчейки = Ложь;
	
КонецПроцедуры

&НаКлиенте
Процедура ЭП_ДобавитьИдею(Команда)

	Отказ = Ложь;	
	Если НЕ ЗначениеЗаполнено(Объект.Ответственный) Тогда	
		ОбщегоНазначенияКлиент.СообщитьПользователю("Не заполнен отвественный",,,,Отказ);
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(Объект.Владелец)Тогда 
		ОбщегоНазначенияКлиент.СообщитьПользователю("Не заполнен проект",,,,Отказ);
	КонецЕсли;
	Если Отказ Тогда
		Возврат;
	КонецЕсли;

	ПараметрыЗаполнения = Новый Структура("Владелец, Ответственный", Объект.Владелец, Объект.Ответственный);	
	ПараметрыОткрытия = Новый Структура("ЗначенияЗаполнения", ПараметрыЗаполнения );	
	ОткрытьФорму("Справочник.Идеи.Форма.ЭП_БыстраяИдея", ПараметрыОткрытия, Элементы.Идеи,,,,, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	
КонецПроцедуры

&НаКлиенте
Процедура ЭП_ИдеиОбработкаЗаписиНового(НовыйОбъект, Источник, СтандартнаяОбработка)
	
	Если Источник.ИмяФормы = "Справочник.Идеи.Форма.ЭП_БыстраяИдея" Тогда
		СтрокаИдеи = Объект.ИдеиИОшибки.Добавить();
		СтрокаИдеи.Идея = НовыйОбъект;
		
		Реквизиты = РеквизитыИдеиОшибки(СтрокаИдеи.Идея);
		
		СтрокаИдеи.СтатусИдеи = Реквизиты.Статус;
		СтрокаИдеи.ОписаниеИдеи = Реквизиты.ОписаниеИдеи;
		СтрокаИдеи.ДоступноЗакрытие = Реквизиты.ДоступноЗакрытие;
		СтрокаИдеи.ИндексКартинкиСтатусаИдеи = ИндексКартинкиСтатусаИдеиОшибки(СтрокаИдеи.СтатусИдеи);
		
		Если ТипЗнч(СтрокаИдеи.Идея) = Тип("СправочникСсылка.Идеи") Тогда
			СтрокаИдеи.ИндексКартинкиИдеяОшибка = 0;
		ИначеЕсли ТипЗнч(СтрокаИдеи.Идея) = Тип("СправочникСсылка.Ошибки") Тогда
			СтрокаИдеи.ИндексКартинкиИдеяОшибка = 1;
		Иначе
			СтрокаИдеи.ИндексКартинкиИдеяОшибка = 99;
		КонецЕсли;
...
Процедура ЭП_ЗаполнитьВерсию()
	...
	Если ВыборкаДетальныеЗаписи.Следующий() Тогда 
		...
		Объект.Версия = ВыборкаДетальныеЗаписи.Ссылка;
...
&НаКлиенте
Процедура ПослеЗакрытияПроизвольнойФормы(Результат, ДопПараметры) Экспорт
	Если ТипЗнч(Результат) = Тип("Структура") Тогда 
		ЭП_Подстатус = Результат.Подстатус;
		ИзменениеЗаголовкаПодстатуса(Строка(Результат.Подстатус));
	КонецЕсли;
	ЭтотОбъект.Прочитать();
КонецПроцедуры

&НаСервереБезКонтекста
Функция ПериодЗаписиРегистра(СсылкаНаОбъект)
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ЭП_ПодстатусыТехническихПроектовСрезПоследних.Период КАК Период
	|ИЗ
	|	РегистрСведений.ЭП_ПодстатусыТехническихПроектов.СрезПоследних(, ТехПроект = &ТехПроект) КАК ЭП_ПодстатусыТехническихПроектовСрезПоследних";
	
	Запрос.УстановитьПараметр("ТехПроект", СсылкаНаОбъект);
	РезультатЗапроса = Запрос.Выполнить();
...
&НаСервере
Процедура ЗаполнитьТекущийПодстатус()
	...
	Если РезультатЗапроса.Пустой() Тогда
		...
		ИзменениеЗаголовкаПодстатуса("Не заполнено"); 
		
	Иначе 
		
		Выборка = РезультатЗапроса.Выбрать(); Выборка.Следующий();
		ЭП_Подстатус = Выборка.Подстатус;
		ИзменениеЗаголовкаПодстатуса(Строка(Выборка.Подстатус)); 
...
Процедура ПослеОтветаНаВопрос(Результат, Параметры) Экспорт
	...
	Если Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыТехническихПроектов.Активен") И Не ЗначениеЗаполнено(ЭП_Подстатус) Тогда
		Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияПроизвольнойФормы", ЭтотОбъект);
		ПараметрыОткрытия = Новый Структура("ТехПроект", Объект.Ссылка);
		ОткрытьФорму("РегистрСведений.ЭП_ПодстатусыТехническихПроектов.ФормаЗаписи", ПараметрыОткрытия, ЭтотОбъект,,,, Оповещение);
	ИначеЕсли Не ЗначениеЗаполнено(ЭП_Подстатус) Тогда
		Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияПроизвольнойФормы", ЭтотОбъект);
		ПараметрыОткрытия = Новый Структура("ТехПроект", Объект.Ссылка);
		ОткрытьФорму("РегистрСведений.ЭП_ПодстатусыТехническихПроектов.ФормаЗаписи", ПараметрыОткрытия, ЭтотОбъект,,,, Оповещение);
	Иначе
...
&НаКлиенте
Процедура ЭП_ПодстатусЗаполнениеНажатие(Элемент)
	 
	 Если Объект.Ссылка.Пустая() Тогда
	
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Для изменения подстатуса необходимо записать объект. Подстатус 'Написание постановки' будет установлен автоматически";        
		Сообщение.Сообщить();                                                 
		Возврат;
	...
	Если ЭтаФорма.Модифицированность Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ПослеОтветаНаВопрос",ЭтотОбъект), 
												НСтр("ru = 'Для продолжения необходимо записать внесенные изменения. Продолжить?'"), 
												РежимДиалогаВопрос.ДаНет);	
...
&НаСервере 
Процедура ИзменениеЗаголовкаПодстатуса(НазваниеПодстатуса)
	
	ЭтаФорма.Элементы.ЭП_ПодстатусЗаполнение.Заголовок = НазваниеПодстатуса;
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьТегиТехПроекта() 
	...
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ОтобранныеТеги.Представление КАК ТегНаименование
	|ИЗ
	|	(ВЫБРАТЬ
	...
	|		И ЭП_ТегиПоТехническимПроектам.ТехническийПроект = &ТехническийПроект) КАК ОтобранныеТеги
	|
	|УПОРЯДОЧИТЬ ПО
	|	ТегНаименование";
	...
	Пока Выборка.Следующий() Цикл 			
		...
		ТегиЗаполнение = ТегиЗаполнение + Выборка.ТегНаименование + "; ";	
...
Процедура ЭП_ТегиЗаполнениеНажатие(Элемент)
	ПараметрыФормы = Новый Структура("Проект, ТехническийПроект", Объект.Владелец, Объект.Ссылка);
...
Процедура ПослеЗакрытияФормыПоискаТегов(Результат, Параметры)  Экспорт
	...
	ЗадатьТегиНаСервере(Результат);
	
КонецПроцедуры

&НаСервере
Процедура ЗадатьТегиНаСервере(Теги)
	
	НачатьТранзакцию();

    Попытка
		Проект = Объект.Владелец;
		ТехническийПроект = Объект.Ссылка;
		
		Блокировка = Новый БлокировкаДанных;
		ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.ЭП_ТегиПоТехническимПроектам");
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
		ЭлементБлокировки.УстановитьЗначение("ТехническийПроект",ТехническийПроект);
		...
		Запрос.Текст = ПолучитьТексЗапроса();
		
		УстановитьПривилегированныйРежим(Истина);
		ПакетРезультатовЗапросов = Запрос.ВыполнитьПакет();
		КоличествоПакетов = ПакетРезультатовЗапросов.Количество();
		Выборка = ПакетРезультатовЗапросов[КоличествоПакетов - 1].Выбрать(); 
		Пока Выборка.Следующий() Цикл
			
			МенеджерЗаписи = РегистрыСведений.ЭП_ТегиПоТехническимПроектам.СоздатьМенеджерЗаписи();
			МенеджерЗаписи.Проект = Проект;
			МенеджерЗаписи.ТехническийПроект = ТехническийПроект;
			МенеджерЗаписи.Тег = Выборка.Тег;
			МенеджерЗаписи.Записать();
			
			МенеджерЗаписи = РегистрыСведений.ЭП_ИзмененияТеговВПроектах.СоздатьМенеджерЗаписи();
			МенеджерЗаписи.Проект = Проект;
			МенеджерЗаписи.Тег = Выборка.Тег;
		...
		Выборка = ПакетРезультатовЗапросов[КоличествоПакетов - 2].Выбрать();
		Пока Выборка.Следующий() Цикл
			НаборЗаписей =  РегистрыСведений.ЭП_ТегиПоТехническимПроектам.СоздатьНаборЗаписей();
			НаборЗаписей.Отбор.Проект.Установить(Проект);
			НаборЗаписей.Отбор.ТехническийПроект.Установить(ТехническийПроект);
			НаборЗаписей.Отбор.Тег.Установить(Выборка.Тег);
	...
	Исключение
		...
		СообщениеПользователю.Текст = "Не удалось установить теги";
		СообщениеПользователю.Сообщить();
    КонецПопытки;	
	
КонецПроцедуры  

Функция ПолучитьТексЗапроса()
	
	Возврат 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ЭП_Теги.Ссылка КАК Тег
	|ПОМЕСТИТЬ ВТ_ТаблицаТеги
	|ИЗ
	|	Справочник.ЭП_Теги КАК ЭП_Теги
	|ГДЕ
	|	ЭП_Теги.Ссылка В(&Теги)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЭП_ТегиПоТехническимПроектам.ТехническийПроект КАК ТехническийПроект,
	|	ЭП_ТегиПоТехническимПроектам.Тег КАК Тег
	|ИЗ
	|	РегистрСведений.ЭП_ТегиПоТехническимПроектам КАК ЭП_ТегиПоТехническимПроектам
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ТаблицаТеги КАК ВТ_ТаблицаТеги
	|		ПО ЭП_ТегиПоТехническимПроектам.Тег = ВТ_ТаблицаТеги.Тег
	|ГДЕ
	|	ЭП_ТегиПоТехническимПроектам.ТехническийПроект = &ТехническийПроект
	|	И ВТ_ТаблицаТеги.Тег ЕСТЬ NULL
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ТаблицаТеги.Тег КАК Тег
	|ИЗ
	|	ВТ_ТаблицаТеги КАК ВТ_ТаблицаТеги
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЭП_ТегиПоТехническимПроектам КАК ЭП_ТегиПоТехническимПроектам
	|		ПО ВТ_ТаблицаТеги.Тег = ЭП_ТегиПоТехническимПроектам.Тег
	|			И (ЭП_ТегиПоТехническимПроектам.ТехническийПроект = &ТехническийПроект)
	|			И (ЭП_ТегиПоТехническимПроектам.Проект = &Проект)
	|ГДЕ
	|	ЭП_ТегиПоТехническимПроектам.Тег ЕСТЬ NULL";
	
	
КонецФункции

&НаКлиенте
Процедура ОбъектыМетаданныхПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	Если НоваяСтрока И Не Копирование Тогда
	
		Элемент.ТекущиеДанные.Порядок = 1;		
	
	КонецЕсли;
КонецПроцедуры

&НаСервере
Процедура ЭП_ЗависимостиТехническийПроектОбработкаВыбораНаСервере(Ссылка)
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ТехническиеПроекты.Статус КАК Статус,
		|	ЕСТЬNULL(ЭП_ПодстатусыТехническихПроектовСрезПоследних.Подстатус, 0) КАК Подстатус
		|ИЗ
		|	Справочник.ТехническиеПроекты КАК ТехническиеПроекты
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЭП_ПодстатусыТехническихПроектов.СрезПоследних(&Дата, ТехПроект = &Ссылка) КАК ЭП_ПодстатусыТехническихПроектовСрезПоследних
		|		ПО ТехническиеПроекты.Ссылка = ЭП_ПодстатусыТехническихПроектовСрезПоследних.ТехПроект
		|ГДЕ
		|	ТехническиеПроекты.Ссылка = &Ссылка";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		НоваяСтрока = Объект.ЭП_Зависимости.НайтиПоИдентификатору(Элементы.ЭП_Зависимости.ТекущаяСтрока);
		НоваяСтрока.ТехническийПроект = Ссылка;
		НоваяСтрока.Статус = ВыборкаДетальныеЗаписи.Статус;
		НоваяСтрока.Подстатус = ВыборкаДетальныеЗаписи.Подстатус;		
	КонецЦикла;
КонецПроцедуры

&НаКлиенте
Процедура ЭП_ЗависимостиТехническийПроектОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, СтандартнаяОбработка)
	ЭП_ЗависимостиТехническийПроектОбработкаВыбораНаСервере(ВыбранноеЗначение);	
КонецПроцедуры

&НаСервере
Процедура ПеречитатьСтатусыНаСервере()
	МассивТехПроектов = Объект.ЭП_Зависимости.Выгрузить(,"ТехническийПроект").ВыгрузитьКолонку("ТехническийПроект");
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ЭП_ПодстатусыТехническихПроектовСрезПоследних.ТехПроект.Ссылка КАК ТехническийПроект,
		|	ЭП_ПодстатусыТехническихПроектовСрезПоследних.ТехПроект.Статус КАК Статус,
		|	ЭП_ПодстатусыТехническихПроектовСрезПоследних.Подстатус КАК Подстатус
		|ИЗ
		|	РегистрСведений.ЭП_ПодстатусыТехническихПроектов.СрезПоследних(, ТехПроект В (&МассивТехПроектов)) КАК ЭП_ПодстатусыТехническихПроектовСрезПоследних";
	Запрос.УстановитьПараметр("МассивТехПроектов", МассивТехПроектов);
	
	Выборка = Запрос.Выполнить().Выбрать();	
	Пока Выборка.Следующий() Цикл    
		СтрокаТЧ = Объект.ЭП_Зависимости.НайтиСтроки(Новый Структура("ТехническийПроект", Выборка.ТехническийПроект));
		СтрокаТЧ[0].ТехническийПроект = Выборка.ТехническийПроект;
		СтрокаТЧ[0].Статус = Выборка.Статус;
		СтрокаТЧ[0].Подстатус = Выборка.Подстатус;
	КонецЦикла;	
КонецПроцедуры

&НаКлиенте
Процедура ПеречитатьСтатусы(Команда)	
	ПеречитатьСтатусыНаСервере();
КонецПроцедуры

&НаКлиенте
Процедура ЭП_ЗависимостиТехническийПроектПриИзменении(Элемент)
	ТехПроект = Элементы.ЭП_Зависимости.ТекущиеДанные.ТехническийПроект;

	ЭП_ЗависимостиТехническийПроектПриИзмененииНаСервере(ТехПроект);		
КонецПроцедуры

&НаСервере
Процедура ЭП_ЗависимостиТехническийПроектПриИзмененииНаСервере(ТехПроект)	
	НайденнаяСтрока = Объект.ЭП_Зависимости.НайтиСтроки(Новый Структура("ТехническийПроект", ТехПроект)); 
	
	Если НайденнаяСтрока.Количество() > 1 Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = СтрШаблон("Технический проект ""%1"" уже есть в таблице зависимостей.", 
									НайденнаяСтрока[1].ТехническийПроект);
		Сообщение.Сообщить(); 
		Объект.ЭП_Зависимости.Удалить(НайденнаяСтрока[1]);	
	КонецЕсли;	
КонецПроцедуры

&НаКлиенте
Процедура ЭП_ЗаполнитьМетаданные(Команда)
	
	ЭП_ЗаполнитьМетаданныеНаСервере();	
 
КонецПроцедуры 

&НаСервере
Процедура ЭП_ЗаполнитьМетаданныеНаСервере()

	СтрокаИзЗаметок = Объект.Заметки;
	МассивПодстрок = МассивПодстрокОбъектов();	
	СоединенныеПодстроки = СтрСоединить(МассивПодстрок, "|");
	РегулярноеВыражение = "(?<!Объект не был изменен:\s)(" + СоединенныеПодстроки + ")\.[а-яА-ЯA-Z0-9_.]+";
	РезультатПоиска = СтрНайтиВсеПоРегулярномуВыражению(СтрокаИзЗаметок, РегулярноеВыражение, , Истина);

	Если Не ЗначениеЗаполнено(СтрокаИзЗаметок) Или Не ЗначениеЗаполнено(РезультатПоиска) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Не заполнены заметки или в заметках не найдены объекты для заполнения.";
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;	
	...
	Для каждого СтрокаПоиска Из РезультатПоиска Цикл 
		...
		Наименование = СтрРазделить(СтрокаПоиска.Значение, ".", Ложь);
		ВидОбъекта = ?(ВидОбъектаВЕдЧисле(Наименование[0]) = Неопределено, "", ВидОбъектаВЕдЧисле(Наименование[0]));
		Имя = ?(Наименование.Количество() > 0, Наименование[1], "");
		
		Если Не ЗначениеЗаполнено(ВидОбъекта) Или Не ЗначениеЗаполнено(Имя) Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = "Не удалось найти объект " + СтрокаПоиска.Значение + ". Проверьте корректность записи.";
			Сообщение.Сообщить();
			Возврат;
		КонецЕсли;
		
		Группа = Справочники.ОбъектыМетаданных.НайтиПоРеквизиту("ВидОбъектаМетаданныхВЕдЧисле", ВидОбъекта, , Объект.Владелец);
		ОбъектМетаданных = Справочники.ОбъектыМетаданных.НайтиПоРеквизиту("Имя", Имя, Группа, Объект.Владелец);
		
		Если ЗначениеЗаполнено(ОбъектМетаданных) Тогда
			Если Объект.ОбъектыМетаданных.НайтиСтроки(Новый Структура("ОбъектМетаданных", ОбъектМетаданных)).Количество() > 0 Тогда
				Продолжить;
			КонецЕсли;
			НоваяСтрокаТЧ = Объект.ОбъектыМетаданных.Добавить();
			
			НоваяСтрокаТЧ.ОбъектМетаданных = ОбъектМетаданных.Ссылка;
			НоваяСтрокаТЧ.Порядок = 1;
			
			Модифицированность = Истина;
		Иначе	
			НовыйОбъект = Справочники.ОбъектыМетаданных.СоздатьЭлемент(); 
			
			НовыйОбъект.Родитель = Группа;
			НовыйОбъект.Владелец = Объект.Владелец;
			НовыйОбъект.Имя = Имя;
			НовыйОбъект.РазделПроекта = Объект.РазделПроекта;
			НовыйОбъект.Записать();
			
			НоваяСтрокаТЧ = Объект.ОбъектыМетаданных.Добавить();
			
			НоваяСтрокаТЧ.ОбъектМетаданных = НовыйОбъект.Ссылка;
			НоваяСтрокаТЧ.Порядок = 1;
			
			Модифицированность = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = ВидОбъекта + " " + Имя + " был записан и добавлен в табличную часть.";
			Сообщение.Сообщить();
		КонецЕсли;
...
&НаСервере
Процедура ОбъектыМетаданныхПриИзмененииНаСервере(ОбъектМетаданных)
	
	НайденнаяСтрока = Объект.ОбъектыМетаданных.НайтиСтроки(Новый Структура("ОбъектМетаданных", ОбъектМетаданных)); 
	
	Если НайденнаяСтрока.Количество() > 1 Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = СтрШаблон("Объект метаданных ""%1"" уже есть в таблице.", 
			НайденнаяСтрока[1].ОбъектМетаданных.Имя);
		Сообщение.Сообщить();
		
		Объект.ОбъектыМетаданных.Удалить(НайденнаяСтрока[1]);
	КонецЕсли;
...
Процедура ОбъектыМетаданныхПриИзменении(Элемент)
	...
	Если Элементы.ОбъектыМетаданных.ТекущиеДанные = Неопределено Тогда
		Возврат;	
	КонецЕсли;
	
	ОбъектМетаданных = Элементы.ОбъектыМетаданных.ТекущиеДанные.ОбъектМетаданных;
	ОбъектыМетаданныхПриИзмененииНаСервере(ОбъектМетаданных);
	
КонецПроцедуры

Функция МассивПодстрокОбъектов()
	
	МассивПодстрок = Новый Массив;
	МассивПодстрок.Добавить("Общи(й|е)Модул(ь|и)");
	МассивПодстрок.Добавить("Параметры?Сеанса");
	МассивПодстрок.Добавить("Общи(й|е)Реквизиты?");
	МассивПодстрок.Добавить("Планы?Обмен(а|ов)"); 
	МассивПодстрок.Добавить("Общи(й|е)Макеты?");
	МассивПодстрок.Добавить("Критери(и|й)Отбор(а|ов)");
	МассивПодстрок.Добавить("Подписк(и|а)НаСобыти(е|я)");
	МассивПодстрок.Добавить("Функциональн(ая|ые)Опци(я|и)");
	МассивПодстрок.Добавить("Параметры?Функциональн(ых|ой)Опци(и|й)");
	МассивПодстрок.Добавить("Определяем(ый|ые)Типы?");
	МассивПодстрок.Добавить("Хранилищ(а|е)Настро(йки|ек)");
	МассивПодстрок.Добавить("Определяем(ый|ые)Типы?");
	МассивПодстрок.Добавить("Общ(ая|ие)Команд(а|ы)");
	МассивПодстрок.Добавить("Групп(а|ы)Команды?");
	МассивПодстрок.Добавить("Общ(ая|ие)Картинк(а|и)");
	МассивПодстрок.Добавить("Пакеты?XDTO");
	МассивПодстрок.Добавить("WebСервисы?");
	МассивПодстрок.Добавить("HTTPСервисы?");
	МассивПодстрок.Добавить("WSСсылк(а|и)");
	МассивПодстрок.Добавить("Элементы?Стиля");
	МассивПодстрок.Добавить("Стил(ь|и)"); 
	МассивПодстрок.Добавить("Языки?");
	МассивПодстрок.Добавить("Общ(ая|ий|ие)Форм(а|ы)");
	МассивПодстрок.Добавить("Регламентн(ы|о)еЗадани(я|е)");
	МассивПодстрок.Добавить("Рол(ь|и)");
	МассивПодстрок.Добавить("Констант(а|ы)");
	МассивПодстрок.Добавить("Документы?");
	МассивПодстрок.Добавить("Справочник?");
	МассивПодстрок.Добавить("Журналы?Документ(а|ов)");
	МассивПодстрок.Добавить("Перечислени(я|е)");
	МассивПодстрок.Добавить("Отчеты?");
	МассивПодстрок.Добавить("Обработк(а|и)");
	МассивПодстрок.Добавить("Планы?(ВидовХарактеристик|Счетов|ВидовРасчета)");
	МассивПодстрок.Добавить("Регистры?(Сведени(й|я)|Накоплени(й|я)|Бухгалтерии|Расчет(а|ов))");
	МассивПодстрок.Добавить("БизнесПроцессы?");
	МассивПодстрок.Добавить("Задач(а|и)");
	МассивПодстрок.Добавить("Общ(ая|ии|ие)Картинк(а|и)");
	МассивПодстрок.Добавить("Внешни(й|е)Источники?Данных");
	
	Возврат МассивПодстрок; 
	
КонецФункции

Функция ВидОбъектаВЕдЧисле(Ключ)
	
	ЕдинственноеЧисло = Новый Соответствие;
	
	ЕдинственноеЧисло.Вставить("Документы", "Документ");
	ЕдинственноеЧисло.Вставить("Документ", "Документ");
	ЕдинственноеЧисло.Вставить("ПараметрыСеанса", "ПараметрСеанса");
	ЕдинственноеЧисло.Вставить("ПараметрСеанса", "ПараметрСеанса");
	ЕдинственноеЧисло.Вставить("ОбщиеРеквизиты", "ОбщийРеквизит");
	ЕдинственноеЧисло.Вставить("ОбщийРеквизиты", "ОбщийРеквизит");
	ЕдинственноеЧисло.Вставить("ОбщиеРеквизит", "ОбщийРеквизит");
	ЕдинственноеЧисло.Вставить("ОбщийРеквизит", "ОбщийРеквизит");
	ЕдинственноеЧисло.Вставить("ПланыОбмена", "ПланОбмена");
	ЕдинственноеЧисло.Вставить("ПланыОбменов", "ПланОбмена");
	ЕдинственноеЧисло.Вставить("ПланОбменов", "ПланОбмена");
	ЕдинственноеЧисло.Вставить("ПланОбмена", "ПланОбмена");
	ЕдинственноеЧисло.Вставить("КритерийОтборов", "КритерийОтбора");
	ЕдинственноеЧисло.Вставить("КритерииОтборов", "КритерийОтбора");
	ЕдинственноеЧисло.Вставить("КритерииОтбора", "КритерийОтбора");
	ЕдинственноеЧисло.Вставить("КритерийОтбора", "КритерийОтбора");
	ЕдинственноеЧисло.Вставить("ПодпискиНаСобытия", "ПодпискаНаСобытие");
	ЕдинственноеЧисло.Вставить("ПодпискиНаСобытие", "ПодпискаНаСобытие");
	ЕдинственноеЧисло.Вставить("ПодпискаНаСобытия", "ПодпискаНаСобытие");
	ЕдинственноеЧисло.Вставить("ПодпискаНаСобытие", "ПодпискаНаСобытие");
	ЕдинственноеЧисло.Вставить("ФункциональныеОпция", "ФункциональнаяОпция");
	ЕдинственноеЧисло.Вставить("ФункциональныеОпции", "ФункциональнаяОпция");
	ЕдинственноеЧисло.Вставить("ФункциональнаяОпции", "ФункциональнаяОпция");
	ЕдинственноеЧисло.Вставить("ФункциональнаяОпция", "ФункциональнаяОпция");
	ЕдинственноеЧисло.Вставить("ПараметрыФункциональнойОпции", "ПараметрФункциональнойОпции");
	ЕдинственноеЧисло.Вставить("ПараметрыФункциональныхОпции", "ПараметрФункциональнойОпции");
	ЕдинственноеЧисло.Вставить("ПараметрыФункциональныхОпций", "ПараметрФункциональнойОпции");
	ЕдинственноеЧисло.Вставить("ПараметрФункциональнойОпции", "ПараметрФункциональнойОпции");
	ЕдинственноеЧисло.Вставить("ПараметрФункциональныхОпции", "ПараметрФункциональнойОпции");
	ЕдинственноеЧисло.Вставить("ПараметрФункциональныхОпций", "ПараметрФункциональнойОпции");
	ЕдинственноеЧисло.Вставить("ПараметрФункциональнойОпции", "ПараметрФункциональнойОпции");
	ЕдинственноеЧисло.Вставить("ОпределяемыеТип", "ОпределяемыйТип");
	ЕдинственноеЧисло.Вставить("ОпределяемыеТипы", "ОпределяемыйТип");
	ЕдинственноеЧисло.Вставить("ОпределяемыйТипы", "ОпределяемыйТип");
	ЕдинственноеЧисло.Вставить("ОпределяемыйТип", "ОпределяемыйТип");
	ЕдинственноеЧисло.Вставить("ХранилищаНастроек", "ХранилищеНастроек");
	ЕдинственноеЧисло.Вставить("ХранилищаНастройки", "ХранилищеНастроек");
	ЕдинственноеЧисло.Вставить("ХранилищеНастройки", "ХранилищеНастроек");
	ЕдинственноеЧисло.Вставить("ХранилищеНастроек", "ХранилищеНастроек");
	ЕдинственноеЧисло.Вставить("ОбщиеКоманды", "ОбщаяКоманда");
	ЕдинственноеЧисло.Вставить("ОбщиеКоманда", "ОбщаяКоманда");
	ЕдинственноеЧисло.Вставить("ОбщаяКоманда", "ОбщаяКоманда");
	ЕдинственноеЧисло.Вставить("ОбщаяКоманда", "ОбщаяКоманда");
	ЕдинственноеЧисло.Вставить("ГруппыКоманд", "ГруппаКоманд");
	ЕдинственноеЧисло.Вставить("ГруппыКоманды", "ГруппаКоманд");
	ЕдинственноеЧисло.Вставить("ГруппаКоманды", "ГруппаКоманд");
	ЕдинственноеЧисло.Вставить("ГруппаКоманд", "ГруппаКоманд");
	ЕдинственноеЧисло.Вставить("ОбщиеКартинка", "ОбщаяКартинка");
	ЕдинственноеЧисло.Вставить("ОбщаяКартинки", "ОбщаяКартинка");
	ЕдинственноеЧисло.Вставить("ОбщиеКартинки", "ОбщаяКартинка");
	ЕдинственноеЧисло.Вставить("ОбщаяКартинка", "ОбщаяКартинка");
	ЕдинственноеЧисло.Вставить("ПакетыXDTO", "ПакетXDTO");
	ЕдинственноеЧисло.Вставить("Пакеты-XDTO", "ПакетXDTO");
	ЕдинственноеЧисло.Вставить("Пакет-XDTO", "ПакетXDTO");
	ЕдинственноеЧисло.Вставить("ПакетXDTO", "ПакетXDTO");
	ЕдинственноеЧисло.Вставить("WebСервисы", "WebСервис");
	ЕдинственноеЧисло.Вставить("Web-Сервисы", "WebСервис");
	ЕдинственноеЧисло.Вставить("Web-Сервис", "WebСервис");
	ЕдинственноеЧисло.Вставить("WebСервис", "WebСервис");
	ЕдинственноеЧисло.Вставить("HTTPСервисы", "HTTPСервис");
	ЕдинственноеЧисло.Вставить("HTTP-Сервисы", "HTTPСервис");
	ЕдинственноеЧисло.Вставить("HTTP-Сервис", "HTTPСервис");
	ЕдинственноеЧисло.Вставить("HTTPСервис", "HTTPСервис");
	ЕдинственноеЧисло.Вставить("WSСсылки", "WSСсылка");
	ЕдинственноеЧисло.Вставить("WS-Ссылки", "WSСсылка");
	ЕдинственноеЧисло.Вставить("WS-Ссылка", "WSСсылка");
	ЕдинственноеЧисло.Вставить("WSСсылка", "WSСсылка");
	ЕдинственноеЧисло.Вставить("ЭлементыСтиля", "ЭлементСтиля");
	ЕдинственноеЧисло.Вставить("ЭлементСтиля", "ЭлементСтиля");
	ЕдинственноеЧисло.Вставить("Стили", "Стиль");
	ЕдинственноеЧисло.Вставить("Стиль", "Стиль"); 
	ЕдинственноеЧисло.Вставить("Языки", "Язык");
	ЕдинственноеЧисло.Вставить("Язык", "Язык");
	ЕдинственноеЧисло.Вставить("ОбщиеМодули", "ОбщийМодуль");
	ЕдинственноеЧисло.Вставить("ОбщиеМодуль", "ОбщийМодуль");
	ЕдинственноеЧисло.Вставить("ОбщийМодуль", "ОбщийМодуль");
	ЕдинственноеЧисло.Вставить("ОбщийМодули", "ОбщийМодуль");
	ЕдинственноеЧисло.Вставить("ОбщаяФормы", "ОбщаяФорма");
	ЕдинственноеЧисло.Вставить("ОбщаяФорма", "ОбщаяФорма");
	ЕдинственноеЧисло.Вставить("ОбщиеФормы", "ОбщаяФорма");
	ЕдинственноеЧисло.Вставить("ОбщиеМакеты", "ОбщийМакет");
	ЕдинственноеЧисло.Вставить("ОбщиеМакет", "ОбщийМакет");
	ЕдинственноеЧисло.Вставить("ОбщийМакеты", "ОбщийМакет");
	ЕдинственноеЧисло.Вставить("ОбщийМакет", "ОбщийМакет");
	ЕдинственноеЧисло.Вставить("РегламентныеЗадание", "РегламентноеЗадание");
	ЕдинственноеЧисло.Вставить("РегламентныеЗадания", "РегламентноеЗадание");
	ЕдинственноеЧисло.Вставить("РегламентноеЗадания", "РегламентноеЗадание");
	ЕдинственноеЧисло.Вставить("РегламентноеЗадание", "РегламентноеЗадание");
	ЕдинственноеЧисло.Вставить("Роли", "Роль");
	ЕдинственноеЧисло.Вставить("Роль", "Роль");
	ЕдинственноеЧисло.Вставить("ОбщиеКартинки", "ОбщаяКартинка");
	ЕдинственноеЧисло.Вставить("ОбщаяКартинки", "ОбщаяКартинка");
	ЕдинственноеЧисло.Вставить("ОбщиеКартинка", "ОбщаяКартинка");
	ЕдинственноеЧисло.Вставить("ОбщаяКартинка", "ОбщаяКартинка");
	ЕдинственноеЧисло.Вставить("Константы", "Константа");
	ЕдинственноеЧисло.Вставить("Константа", "Константа");
	ЕдинственноеЧисло.Вставить("Справочники", "Справочник");
	ЕдинственноеЧисло.Вставить("Справочник", "Справочник");
	ЕдинственноеЧисло.Вставить("ЖурналыДокументов", "ЖурналДокументов");
	ЕдинственноеЧисло.Вставить("ЖурналДокументов", "ЖурналДокументов");
	ЕдинственноеЧисло.Вставить("Перечисления", "Перечисление");
	ЕдинственноеЧисло.Вставить("Перечисление", "Перечисление");
	ЕдинственноеЧисло.Вставить("Отчеты", "Отчет");
	ЕдинственноеЧисло.Вставить("Отчет", "Отчет");
	ЕдинственноеЧисло.Вставить("Обработки", "Обработка");
	ЕдинственноеЧисло.Вставить("Обработка", "Обработка");
	ЕдинственноеЧисло.Вставить("ПланыВидовХарактеристик", "ПланВидовХарактеристик");
	ЕдинственноеЧисло.Вставить("ПланВидовХарактеристик", "ПланВидовХарактеристик");
	ЕдинственноеЧисло.Вставить("ПланыСчетов", "ПланСчетов");
	ЕдинственноеЧисло.Вставить("ПланСчетов", "ПланСчетов");
	ЕдинственноеЧисло.Вставить("ПланыВидовРасчета", "ПланВидовРасчета");
	ЕдинственноеЧисло.Вставить("ПланВидовРасчета", "ПланВидовРасчета");
	ЕдинственноеЧисло.Вставить("РегистрыСведений", "РегистрСведений");
	ЕдинственноеЧисло.Вставить("РегистрСведений", "РегистрСведений");
	ЕдинственноеЧисло.Вставить("РегистрыСведения", "РегистрСведений");
	ЕдинственноеЧисло.Вставить("РегистрСведения", "РегистрСведений");
	ЕдинственноеЧисло.Вставить("РегистрыНакопления", "РегистрНакопления");
	ЕдинственноеЧисло.Вставить("РегистрНакопления", "РегистрНакопления");
	ЕдинственноеЧисло.Вставить("РегистрыНакоплений", "РегистрНакопления");
	ЕдинственноеЧисло.Вставить("РегистрНакоплений", "РегистрНакопления");
	ЕдинственноеЧисло.Вставить("РегистрыБухгалтерии", "РегистрБухгалтерии");
	ЕдинственноеЧисло.Вставить("РегистрБухгалтерии", "РегистрБухгалтерии");
	ЕдинственноеЧисло.Вставить("РегистрыРасчета", "РегистрРасчета");
	ЕдинственноеЧисло.Вставить("РегистрРасчета", "РегистрРасчета");
	ЕдинственноеЧисло.Вставить("РегистрРасчетов", "РегистрРасчета");
	ЕдинственноеЧисло.Вставить("РегистрРасчета", "РегистрРасчета");
	ЕдинственноеЧисло.Вставить("БизнесПроцессы", "БизнесПроцесс");
	ЕдинственноеЧисло.Вставить("БизнесПроцесс", "БизнесПроцесс");
	ЕдинственноеЧисло.Вставить("Задачи", "Задача");
	ЕдинственноеЧисло.Вставить("Задача", "Задача");
	ЕдинственноеЧисло.Вставить("ВнешниеИсточникиДанных", "ВнешнийИсточникДанных");
	ЕдинственноеЧисло.Вставить("ВнешнийИсточникДанных", "ВнешнийИсточникДанных");
	
	Возврат ЕдинственноеЧисло[Ключ];
	
КонецФункции

&НаКлиенте
Процедура ДекорацияДокументРеализацииНажатие(Элемент)
	
	Если ЗначениеЗаполнено(Объект.ЭП_РеализацияУслуг) Тогда
		ПараметрыФормы = Новый Структура("Ключ", Объект.ЭП_РеализацияУслуг); 
		ОткрытьФорму("Документ.ЭП_РеализацияУслуг.ФормаОбъекта", ПараметрыФормы, ЭтаФорма,,
		,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);   
	КонецЕсли;  
...
&НаКлиенте
Процедура ИзменитьРеализацию(Команда)
	
	Отбор = Новый Структура("Проект", Объект.Владелец);
	ПараметрыФормы = Новый Структура("РежимВыбора, Отбор, МножественныйВыбор, ЗакрыватьПриВыборе", 
	Истина, Отбор, Ложь, Истина); 
	ОткрытьФорму("Документ.ЭП_РеализацияУслуг.ФормаВыбора", ПараметрыФормы, ЭтаФорма,,
	,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца); 
...
Процедура ЭП_АрхивныйПриИзменении(Элемент)
	...
	Иначе
		...
		Объект.ЭП_ДатаПомещенияВАрхив = Дата(1, 1, 1);
