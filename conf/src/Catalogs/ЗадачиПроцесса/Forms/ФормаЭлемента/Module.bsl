...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтатусДо = Объект.ЭП_СтатусЗадачи;
	// Глинский А.А., ЭП-0024441, 22.01.2025 {
	Если НЕ Пользователи.РолиДоступны("ЭП_ТестированиеДоработкиЗадачиПРоцесса", ПараметрыСеанса.ТекущийПользователь, Истина) Тогда		
		Элементы.ЭП_ТегиЗаполнение.Видимость = Ложь;
	КонецЕсли;
	// Глинский А.А., ЭП-0024441, 22.01.2025 }  
	...
	// Глинский А.А., ЭП-0025054, 18.02.2025 {															   
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	СУММА(УчетВремени.Длительность / 3600) КАК Длительность,
		|	ЕСТЬNULL(УчетВремени.Пользователь.ЭП_Сотрудник.Департамент.Представление, """") КАК ПользовательЭП_СотрудникДепартаментПредставление
		|ИЗ
		|	РегистрНакопления.УчетВремени КАК УчетВремени
		|ГДЕ
		|	УчетВремени.Задача = &Задача
		|	И НЕ УчетВремени.Задача = ЗНАЧЕНИЕ(Справочник.ЗадачиПроцесса.ПустаяСсылка)
		|
		|СГРУППИРОВАТЬ ПО
		|	ЕСТЬNULL(УчетВремени.Пользователь.ЭП_Сотрудник.Департамент.Представление, """")"; 
	// Глинский А.А., ЭП-0025054, 18.02.2025 }
	...
	ДобавляемыйРеквизитСумма = Новый Массив;
	ДобавляемыеДепартаменты = Новый Массив;
	Если НЕ Параметры.Ключ.Пустая() Тогда
		
		Пока Выборка.Следующий() Цикл
			Если Выборка.ПользовательЭП_СотрудникДепартаментПредставление = "" Тогда
			
				Продолжить;	
			
			КонецЕсли;
			ДепартаментИмя = Выборка.ПользовательЭП_СотрудникДепартаментПредставление;
			Департамент = СтрЗаменить(ДепартаментИмя," ","");
			ДобавляемыеДепартаменты.Добавить(ДепартаментИмя);
			
			НовыйРеквизит = Новый РеквизитФормы(Департамент, Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(5),,));
			ДобавляемыеРеквизиты.Добавить(НовыйРеквизит); 
			
		КонецЦикла; 
		
		НовыйРеквизит = Новый РеквизитФормы("СуммарныеЧасы", Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(5),,));
		ДобавляемыйРеквизитСумма.Добавить(НовыйРеквизит); 
		
		ИзменитьРеквизиты(ДобавляемыеРеквизиты);
		ИзменитьРеквизиты(ДобавляемыйРеквизитСумма); 
	КонецЕсли;
	Счетчик = 0;
	Для Каждого Элемент Из ДобавляемыеРеквизиты Цикл 
		
		НовыйЭлемент = ЭтаФорма.Элементы.Добавить(Элемент.Имя, Тип("ПолеФормы"), Элементы.ФактическиеТруозатраты);
		НовыйЭлемент.Вид = ВидПоляФормы.ПолеНадписи;
		НовыйЭлемент.ПутьКДанным = Элемент.Имя; 
		НовыйЭлемент.Заголовок = ДобавляемыеДепартаменты[Счетчик];
		НовыйЭлемент.АвтоМаксимальнаяШирина = Ложь;
		НовыйЭлемент.МаксимальнаяШирина = 4;
	...
	Если ДобавляемыеРеквизиты.Количество() > 0 Тогда 
		...
		НовыйЭлемент = ЭтаФорма.Элементы.Добавить("СуммарныеЧасы", Тип("ПолеФормы"), Элементы.ГруппаСуммарно);
		НовыйЭлемент.Вид = ВидПоляФормы.ПолеНадписи;
		НовыйЭлемент.ПутьКДанным = "СуммарныеЧасы";
	...
	// Глинский А.А., ЭП-0024441, 22.01.2025 {
	Если Пользователи.РолиДоступны("ЭП_ТестированиеДоработкиЗадачиПРоцесса", ПараметрыСеанса.ТекущийПользователь, Истина) Тогда 
			
		КрайняяДатаДоИзменения = Объект.КрайняяДатаОкончания; 
		
	КонецЕсли;
	// Глинский А.А., ЭП-0024441, 22.01.2025 }
	
	ЗаполнитьРеквезитыНаСервере();
	
	ЗаполнитьПланЧасовНаСервере();
	
	ЗаполнитьКоличествоИтерацийНаСервере();
	
    ЗаполнитьТегиЗадачиПроцесса();
	
	УстановитьПроектНаСервере();
	
КонецПроцедуры

&НаСервере
Процедура УстановитьПроектНаСервере()

	Если ЗначениеЗаполнено(Объект.Предмет) И (ТипЗнч(Объект.Предмет) = Тип("СправочникСсылка.ТехническиеПроекты") ИЛИ ТипЗнч(Объект.Предмет) = Тип("СправочникСсылка.Ошибки")) Тогда
	
		Проект = Объект.Предмет.Владелец;		
...
&НаСервере
Процедура ЗаполнитьРеквезитыНаСервере()
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	СУММА(УчетВремени.Длительность / 3600) КАК Длительность,
		|	УчетВремени.Пользователь.ЭП_Сотрудник.Департамент.Представление КАК ПользовательЭП_СотрудникДепартаментПредставление
		|ИЗ
		|	РегистрНакопления.УчетВремени КАК УчетВремени
		|ГДЕ
		|	УчетВремени.Задача = &Задача
		|
		|СГРУППИРОВАТЬ ПО
		|	УчетВремени.Пользователь.ЭП_Сотрудник.Департамент.Представление";
	...
	Пока Выборка.Следующий() Цикл 
		...
		Департамент = Выборка.ПользовательЭП_СотрудникДепартаментПредставление;
		Департамент = СтрЗаменить(Департамент," ","");
		
		СуммаЧасов = СуммаЧасов + Выборка.Длительность;
		Для Каждого Реквезит Из ЭтаФорма.ПолучитьРеквизиты() Цикл
			Если Реквезит.Имя = Департамент Тогда
				ЭтаФорма[Реквезит.Имя] = СтрШаблон("%1 ч.", Выборка.Длительность);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	
	Для Каждого Реквезит Из ЭтаФорма.ПолучитьРеквизиты() Цикл
			Если Реквезит.Имя = "СуммарныеЧасы" Тогда
				ЭтаФорма[Реквезит.Имя] = СтрШаблон("%1 ч.", СуммаЧасов);
			КонецЕсли;
		КонецЦикла;
...
&НаСервере 
Процедура ЗаполнитьПланЧасовНаСервере()
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	КОЛИЧЕСТВО(ЭП_ПлановыЧасыПоЗадачам.ЗадачаПроцесса) КАК КоличествоИтераций
		|ИЗ
		|	РегистрСведений.ЭП_ПлановыЧасыПоЗадачам КАК ЭП_ПлановыЧасыПоЗадачам
		|ГДЕ
		|	ЭП_ПлановыЧасыПоЗадачам.ЗадачаПроцесса = &ЗадачаПроцесса";
	...
	Пока Выборка.Следующий() Цикл
    	Итерации = Выборка.КоличествоИтераций;
	КонецЦикла;	
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда 
		...
		Пока Выборка.Следующий() Цикл
			ЧасыПлановые = СтрШаблон("%1 ч. (%2)",Выборка.ПлановыеЧасы, Итерации);  
		КонецЦикла; 
		
	Иначе  

		ЧасыПлановые = СтрШаблон("%1 ч.", 0);
...
&НаСервере 
Процедура ЗаполнитьКоличествоИтерацийНаСервере()
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ТекущийОбъект.Статус <> СтатусПоСтрокеПротокола 
		Или (ТекущийОбъект.ИсполнительПорученияПоЗадаче <> НаправленаПоСтрокеПротокола И ТекстПоручения <> "") 
	...
	Если Пользователи.РолиДоступны("ЭП_ТестированиеДоработкиЗадачиПРоцесса", ПараметрыСеанса.ТекущийПользователь, Истина) Тогда
		...
		Если СрокиВыполненияИзменены Тогда    
			
			Комментарий = СформироватьКомментарийИзмененияКрайнихСроков(КрайняяДатаДоИзменения, ТекущийОбъект.КрайняяДатаОкончания, ТекущийОбъект.ЭП_СтатусЗадачи);
			
			ДобавитьЗаписьВПротокол(ТекущийОбъект, ТекущийПользователь, Комментарий); 
			
			СрокиВыполненияИзменены = Ложь;	
			
		КонецЕсли;
		
	КонецЕсли;
	// Глинский А.А., ЭП-0024441, 22.01.2025 }
	Если НЕ СтатусДо = Объект.ЭП_СтатусЗадачи Тогда
		
		Комментарий = СформироватьКомментарийИзмененияСтатуса(СтатусДо, Объект.ЭП_СтатусЗадачи);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	КонецЕсли;
	// Конец СтандартныеПодсистемы.Свойства
	Если ИмяСобытия = "ДобавитьИтерациюПроекта" Тогда
		ЗаполнитьКоличествоИтерацийНаСервере();
		ЗаполнитьПланЧасовНаСервере();
	КонецЕсли;
КонецПроцедуры
...
Процедура ПротоколВыборЗавершение(РезультатДействия, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(РезультатДействия) = Тип("Структура") И РезультатДействия.Свойство("ТекстПоручения") Тогда
		
		СтрокаТЧ = ДополнительныеПараметры.СтрокаПротокола;
		СтрокаТЧ.ТекстПоручения = РезультатДействия.ТекстПоручения;
		
		Если РезультатДействия.Свойство("Исполнитель") Тогда
			СтрокаТЧ.ИсполнительПоручения = РезультатДействия.Исполнитель;
		КонецЕсли;
...
&НаСервере
Функция ПараметрыОткрытияПротоколаРесурса(КодСогласуемогоРесурса)
	...
	ВыборкаТекущее =  Результат[3].Выбрать();
	Если ВыборкаТекущее.Следующий() Тогда
		ТекущееЗапланировано = ВыборкаТекущее.Запланировано;
		ТекущееСогласовано   = ВыборкаТекущее.Согласовано;
	КонецЕсли;
	
	ТекущаяДата = ТекущаяДатаСеанса();
	
	Если ТекущееЗапланировано <> 0 Тогда
		...
		НоваяСтрока.Количество               = ТекущееЗапланировано;
...
&НаКлиенте
Процедура ОткрытьПротоколСогласованияРесурсов(КодВидаРесурса)
	
	ПараметрыОткрытия = ПараметрыОткрытияПротоколаРесурса(КодВидаРесурса);
...
Процедура ПриОбработкеНавигационнойСсылкиХодСогласования(НавигационнаяСсылкаФорматированнойСтроки)
	...
	Если СтрНайти(НавигационнаяСсылкаФорматированнойСтроки,"ПротоколСогласования") Тогда
		...
		ОткрытьПротоколСогласованияРесурсов(КодВидаРесурсаПоПостфиксуСтроки(НавигационнаяСсылкаФорматированнойСтроки));
...
Процедура ОтобразитьИнформациюПланХодСогласованияПоВидуРесурса(ПараметрыОтображения)
	...
	Элементы[ПараметрыОтображения.ИмяЭлементаЗапланировано].ТолькоПросмотр = (ЭтотОбъект[ПараметрыОтображения.ИмяРеквизитаЗапланировано] = 0 
	                                                                         И ПараметрыОтображения.ЕстьПодчиненныеЗначимыеЗадачи);
...
Функция РезультатПониженияСтатуса(Статус)
	...
	Если Не ДанныеДляПонижения.ТребуетсяИзменениеДругихЗадач 
		...
		ИначеЕсли ТекущийПользователь <> Объект.Исполнитель
			И Не ЗначениеЗаполнено(Объект.ИсполнительПорученияПоЗадаче) Тогда
...
&НаКлиенте
Процедура ИзменитьПланЧасов(Команда)  
	
	ПарметрыПодбора = Новый Структура;
	ПарметрыПодбора.Вставить("ЗадачаПроцесса", Объект.Ссылка);
	ОткрытьФорму("РегистрСведений.ЭП_ПлановыЧасыПоЗадачам.ФормаЗаписи", ПарметрыПодбора, ЭтаФорма, , , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ИсторияИзменений(Команда)
	ПарметрыПодбора = Новый Структура;
	ПарметрыПодбора.Вставить("ЗадачаПроцесса", Объект.Ссылка);
	ОткрытьФорму("РегистрСведений.ЭП_ПлановыЧасыПоЗадачам.Форма.ФормаЗадач", ПарметрыПодбора, ЭтаФорма, , , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры

&НаСервере
Процедура ЭП_ЗаполнитьТегиЗадачиПроцесса() 
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ЗадачаПроцесса", Объект.Ссылка); 
	Запрос.УстановитьПараметр("Проект", Объект.Предмет.Владелец);
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	|		И ЭП_ТегиПоЗадачамПроцесса.ЗадачаПроцесса = &ЗадачаПроцесса) КАК ОтобранныеТеги
	|
	|УПОРЯДОЧИТЬ ПО
	|	ТегНаименование";
...
&НаСервере
Функция ЭП_ПолучитьСсылкуНаПроект()
	
	// Рощектаев Д.Е., ЭП-0024441, 23.01.2025 {
	//Возврат Объект.Предмет.Владелец.Ссылка; 
	Возврат Объект.Предмет.Владелец;
	// Рощектаев Д.Е., ЭП-0024441, 23.01.2025 }
...
Процедура ЭП_ТегиЗаполнениеНажатие(Элемент) 
	Проект = ЭП_ПолучитьСсылкуНаПроект();
	ПараметрыФормы = Новый Структура("Проект, ЗадачаПроцесса", Проект, Объект.Ссылка);    
	ОписаниеОповещенияОЗакрытии = Новый ОписаниеОповещения("ЭП_ПослеЗакрытияФормыПоискаТегов", ЭтотОбъект);
	ОткрытьФорму("ОбщаяФорма.ЭП_ФормаПоискаТегов", ПараметрыФормы, ЭтаФорма, , , , ОписаниеОповещенияОЗакрытии);
КонецПроцедуры

&НаКлиенте
Процедура ЭП_ПослеЗакрытияФормыПоискаТегов(Результат, Параметры)  Экспорт
	
	Если ТипЗнч(Результат) = Тип("Неопределено") Тогда
		Возврат;
	КонецЕсли;
	
	ЭП_ЗадатьТегиНаСервере(Результат);
	
КонецПроцедуры

&НаСервере
Процедура ЭП_ЗадатьТегиНаСервере(Теги)
	
	НачатьТранзакцию();

    Попытка
		ЗадачаПроцесса = Объект.Ссылка;
		
		Блокировка = Новый БлокировкаДанных;
		ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.ЭП_ТегиПоЗадачамПроцесса"); 
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
		ЭлементБлокировки.УстановитьЗначение("ЗадачаПроцесса", ЗадачаПроцесса); 
		...
		Пока Выборка.Следующий() Цикл
			...
			МенеджерЗаписи = РегистрыСведений.ЭП_ТегиПоЗадачамПроцесса.СоздатьМенеджерЗаписи(); 
			МенеджерЗаписи.Проект = Объект.Предмет.Владелец;
			МенеджерЗаписи.ЗадачаПроцесса = ЗадачаПроцесса;       
		...
		Пока Выборка.Следующий() Цикл
			НаборЗаписей =  РегистрыСведений.ЭП_ТегиПоЗадачамПроцесса.СоздатьНаборЗаписей();    
		...
		ЭП_ЗаполнитьТегиЗадачиПроцесса();   
...
Функция ПолучитьТексЗапроса()
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЭП_ТегиПоЗадачамПроцесса.ЗадачаПроцесса КАК ЗадачаПроцесса,
	|	ЭП_ТегиПоЗадачамПроцесса.Тег КАК Тег
	|ИЗ
	|	РегистрСведений.ЭП_ТегиПоЗадачамПроцесса КАК ЭП_ТегиПоЗадачамПроцесса
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ТаблицаТеги КАК ВТ_ТаблицаТеги
	|		ПО ЭП_ТегиПоЗадачамПроцесса.Тег = ВТ_ТаблицаТеги.Тег
	|ГДЕ
	|	ЭП_ТегиПоЗадачамПроцесса.ЗадачаПроцесса = &ЗадачаПроцесса
	...
	|		ПО ВТ_ТаблицаТеги.Тег = ЭП_ТегиПоЗадачамПроцесса.Тег
	|			И (ЭП_ТегиПоЗадачамПроцесса.ЗадачаПроцесса = &ЗадачаПроцесса)
	|			И (ЭП_ТегиПоЗадачамПроцесса.Проект = &Проект)
	|ГДЕ
	|	ЭП_ТегиПоЗадачамПроцесса.Тег ЕСТЬ NULL";
...
&НаСервере
Процедура ЗаполнитьТегиЗадачиПроцесса() 
	Если НЕ ЗначениеЗаполнено(Объект.Предмет) Тогда
...
&НаСервере
Процедура КрайняяДатаОкончанияПриИзмененииНаСервере()
	...
	Если НЕ ЗначениеЗаполнено(Объект.КрайняяДатаОкончания) ИЛИ КрайняяДатаДоИзменения = Объект.КрайняяДатаОкончания Тогда
		СрокиВыполненияИзменены = Ложь;
	Иначе
		СрокиВыполненияИзменены = Истина; 
	КонецЕсли;
	// Глинский А.А., ЭП-0024441, 22.01.2025 }
...
&НаКлиенте
Процедура КрайняяДатаОкончанияПриИзменении(Элемент)
	КрайняяДатаОкончанияПриИзмененииНаСервере();		  
КонецПроцедуры

// Глинский А.А., ЭП-0024441, 22.01.2025 {
&НаСервереБезКонтекста
Функция СформироватьКомментарийИзмененияКрайнихСроков(ДатаДо, ДатаПосле, Статус)
	
	Комментарий = "";
	
	ДатаДоТекст = Формат(ДатаДо, "ДФ=dd.MM.yyyy");
	ДатаПослеТекст = Формат(ДатаПосле, "ДФ=dd.MM.yyyy"); 
	
	Если ЗначениеЗаполнено(ДатаДоТекст) Тогда
		
		ТекстДата = СтрШаблон("Крайняя дата задачи изменена с %1 на %2", ДатаДоТекст, ДатаПослеТекст);
	  
	Иначе
								
		ТекстДата = СтрШаблон("Установлена крайняя дата выполнения задачи: %1", ДатаПослеТекст);
	...
	Комментарий = СтрШаблон("%1"
	                        "Статус задачи: %2"
							"Ответственный: %3", 
							ТекстДата, Статус, ПараметрыСеанса.ТекущийПользователь);
								
	Возврат Комментарий; 
	
КонецФункции // Глинский А.А., ЭП-0024441, 22.01.2025 }

&НаСервереБезКонтекста
Функция СформироватьКомментарийИзмененияСтатуса(СтатусДо, Статус)
	
	Комментарий = "";
	Если ЗначениеЗаполнено(СтатусДо) Тогда
		
		ТекстСтатуса = СтрШаблон("Статус изменен с: %1"
								 "на: %2", СтатусДо, Статус);
	  
	Иначе
								
		ТекстСтатуса = СтрШаблон("Установлен статус: %1", Статус);
								
	КонецЕсли;
	
	Комментарий = СтрШаблон("%1"
							"Ответственный: %2", 
							ТекстСтатуса, ПараметрыСеанса.ТекущийПользователь);
...
Функция ПолучитьДопПараметры()
	...
	ДополнительныеПараметры = Новый Структура;
	ЗначениеОтбора = Новый Структура("Владелец", Объект.Предмет.Владелец);
...
//&НаКлиенте
//Процедура ЭП_ВидЗадачиНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
//	
//	//СтандартнаяОбработка = Ложь; 
//	//
...
//	//	Сообщение.Текст = "Укажите Предмет!";
//	//	Сообщение.Сообщить();
//	//	Возврат; 
//	//	
//	//КонецЕсли;
//	//
//	//ОткрытьФорму("Справочник.ЭП_ВидыЗадачи.Форма.ФормаВыбора", ПолучитьДопПараметры(), 
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("СправочникСсылка.ЭП_ВидыЗадачи") Тогда
		Объект.ЭП_ВидЗадачи = ВыбранноеЗначение;
	ИначеЕсли ТипЗнч(ВыбранноеЗначение) = Тип("СправочникСсылка.ЭП_СтатусыЗадач") Тогда
		Объект.ЭП_СтатусЗадачи = ВыбранноеЗначение;
	КонецЕсли;
...
//&НаКлиенте
//Процедура ЭП_СтатусЗадачиНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
...
////	ОткрытьФорму("Справочник.ЭП_СтатусыЗадач.Форма.ФормаВыбора", ПолучитьДопПараметры(), 
////					ЭтаФорма,,,,, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
////					
//КонецПроцедуры
